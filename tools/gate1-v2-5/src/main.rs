...
const ACCEPTANCE: &str = "baseline/testing/GATE1_ACCEPTANCE_V2_5.md";
const AUTHORIZATION: &str = "baseline/testing/GATE1_V2_5_AUTHORIZATION.md";
const AUTHORIZATION_RECORD: &str = "experiments/gate1-v2.5/authorization.json";
const THRESHOLDS: &str = "experiments/gate1-v2.5/threshold_equivalence.json";
const ENVIRONMENT: &str = "experiments/gate1-v2.5/environment.json";
const QUALIFICATION: &str = "experiments/gate1-v2.5/qualification/environment_qualification.json";
...
fn main() -> Result<(), AnyError> {
    ...
    match arguments.as_slice() {
        ...
        [command] if command == "qualify-environment" => qualification::qualify_environment(
            Path::new("target/gate1-v2.5-qualification/qualified-host"),
        ),
        ...
        [command] if command == "archive-v2-4" => {
            let result = history_v2_4::archive()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        ...
        [command] if command == "artifact-hygiene-check" => {
            let result =
                scenario::verify_artifact_hygiene(Path::new("target/gate1-v2.5-dryrun"))?;
            println!("{}", serde_json::to_string_pretty(&result)?);
        ...
        _ => Err("usage: nexa-gate1-v2-5 history-check|governance-negative-tests|status-lint|scenario-independence-check|outcome-transport-check|acceptance-equivalence|h2-projection-check|h2-noise-invariance-check|h2-semantic-sensitivity-check|comparison-policy-check|decision-state-check|topology-finalization-check|artifact-hygiene-check|evidence-static-check|qualify-environment|prefreeze-closure|freeze|validate|supervisor <formal-run-1|formal-run-2|replay>|compare <left> <right>".into()),
    }
}
...
fn acceptance_equivalence() -> Result<(), AnyError> {
    ...
    if weakened
        || !removed.is_empty()
        || !changed.is_empty()
        || fingerprint["experiment_version"] != "gate1-v2.5"
        ...
        || fingerprint["authorized_apparatus_changes"]
            != json!([
                "typed H2 semantic projection",
                "independent performance projection",
                "independent allocation projection",
                "deterministic comparison classification",
                "separated decision state",
                "phase-aware contracts",
                "candidate-tree finalization",
                "raw-derived receipt reconstruction"
            ])
    {
        return Err("Gate 1 v2.5 outcome thresholds are not equivalent to v2".into());
    }
    println!(
        "{}",
...
fn evidence_static_check() -> Result<(), AnyError> {
    let roots = [
        Path::new("tools/gate1-v2-5/src"),
        Path::new("tools/gate1-v2-5-gates/src"),
        Path::new("tools/gate1-v2-5-decision/src"),
...
fn freeze() -> Result<(), AnyError> {
    ...
    let prefreeze_closure = read_json("experiments/gate1-v2.5/prefreeze/prefreeze_closure.json")?;
    if prefreeze_closure["status"] != "PASS"
        || prefreeze_closure["synthetic_artifacts_formal_evidence_usable"] != false
    {
        return Err("Gate 1 v2.5 prefreeze closure is not valid".into());
    }
    ...
    {
        return Err("Gate 1 v2.5 environment is not qualified".into());
    }
    let authorization = read_json(AUTHORIZATION_RECORD)?;
    if authorization["status"] != "AUTHORIZED"
        || authorization["prefreeze_closure"]["hash"]
            != hash_file("experiments/gate1-v2.5/prefreeze/prefreeze_closure.json")?
        ...
        || authorization["provenance_protocol"]["hash"]
            != hash_file("experiments/gate1-v2.5/prefreeze/outcome_transport.json")?
        || authorization["execution_budget"]
            != json!({"formal_run_1": 1, "formal_run_2": 1, "replay": 1, "retry": 0})
    {
        return Err("Gate 1 v2.5 authorization bindings are invalid".into());
    }
    let contract_manifest = read_json("reports/contracts/gate1_v2_5_contracts.json")?;
    if contract_manifest["contracts"]
        .as_array()
        .map_or(0, Vec::len)
        != 48
    {
        return Err("Gate 1 v2.5 Contract Manifest does not cover 48 work packages".into());
    }
    ...
    let runner_hash = stable_value_hash(&json!({
        "runner_lib": hashes["tools/gate1-v2-5/src/lib.rs"],
        "runner_main": hashes["tools/gate1-v2-5/src/main.rs"],
        "gates_main": hashes["tools/gate1-v2-5-gates/src/main.rs"],
        "gates_lib": hashes["tools/gate1-v2-5-gates/src/lib.rs"],
        "decision": hashes["tools/gate1-v2-5-decision/src/main.rs"],
        ...
        ,"qualification": hashes["tools/gate1-v2-5/src/qualification.rs"]
        ,"prefreeze": hashes["tools/gate1-v2-5/src/prefreeze.rs"]
        ,"scenario": hashes["tools/gate1-v2-5/src/scenario.rs"]
        ,"projection": hashes["tools/gate1-v2-5/src/projection.rs"]
        ,"history_v2_4": hashes["tools/gate1-v2-5/src/history_v2_4.rs"]
        ,"fixtures_lib": hashes["tools/gate1-v2-5-fixtures/src/lib.rs"]
        ,"fixtures_main": hashes["tools/gate1-v2-5-fixtures/src/main.rs"]
    }));
    let manifest = json!({
        "schema_version": 3,
        "experiment_version": "gate1-v2.5",
        ...
        "prefreeze_closure_hash": hashes["experiments/gate1-v2.5/prefreeze/prefreeze_closure.json"],
        "supersession_graph_hash": hashes["reports/history/gate1/supersession_graph.json"],
        "sample_hash": sample_hash,
        "sample_hashes": sample_hashes,
        "runner_hash": runner_hash,
        "worker_hash": hashes["tools/gate1-v2-5/src/main.rs"],
        "decision_tool_hash": hashes["tools/gate1-v2-5-decision/src/main.rs"],
        "receipt_generator_hash": hashes["tools/gate1-v2-5-decision/src/main.rs"],
        "h2_benchmark_config_hash": hashes["experiments/gate1/benchmark.json"],
        "contract_manifest_hash": hashes["reports/contracts/gate1_v2_5_contracts.json"],
        "bound_hashes": hashes
    });
    write_json(Path::new(MANIFEST), &manifest)?;
    println!(
        "froze Gate 1 v2.5 manifest with {} bound inputs",
        paths.len()
    );
    Ok(())
}
...
fn bound_input_paths() -> Vec<String> {
    let mut paths = [
        ...
        ENVIRONMENT,
        QUALIFICATION,
        "experiments/gate1-v2.5/qualification/environment_qualification.md",
        ...
        "experiments/gate1-v2.5/fixtures/v2_4_h2/formal-run-1.json",
        "experiments/gate1-v2.5/fixtures/v2_4_h2/formal-run-2.json",
        "experiments/gate1-v2.5/fixtures/v2_4_h2/replay.json",
        ...
        "reports/history/gate1/v2_4/terminal.md",
        ...
        "tools/gate1-v2-5/src/lib.rs",
        "tools/gate1-v2-5/src/main.rs",
        "tools/gate1-v2-5/src/history_v2_4.rs",
        "tools/gate1-v2-5/src/projection.rs",
        "tools/gate1-v2-5/src/qualification.rs",
        "tools/gate1-v2-5/src/prefreeze.rs",
        "tools/gate1-v2-5/src/scenario.rs",
        "tools/gate1-v2-5-fixtures/src/lib.rs",
        "tools/gate1-v2-5-fixtures/src/main.rs",
        "tools/gate1-v2-5-gates/src/main.rs",
        "tools/gate1-v2-5-gates/src/lib.rs",
        "tools/gate1-v2-5-decision/src/main.rs",
        ...
        "reports/contracts/gate1_v2_5_contracts.json",
    ]
    .into_iter()
    .map(str::to_owned)
    .collect::<Vec<_>>();
...
fn supervisor(label: &str) -> Result<(), AnyError> {
    ...
    if result["apparatus_status"] != "PASS" {
        return Err(format!("Gate 1 v2.5 {label} was invalid: {}", result["failures"]).into());
    }
    println!(
        "Gate 1 v2.5 {label}: apparatus PASS, outcomes {}",
...
fn run_h1_mutation(
    ...
    let hand_build = cargo_check_binding(
        ...
        &hand_file,
        &repository_root()
            .join("target/gate1-v2.5-build/h1")
            .join(format!("{id}-hand")),
        false,
    )?;
        ...
        Ok(changed_idl) => {
            ...
            if build.status.success() {
                ...
                let module = nexa_compiler::compile_with_metadata(
                    ...
                    changed_hash,
                    StableId::from_name("gate1-v2.5-h1-schema"),
                )?;
    ...
    for spec in scenario::h2_cleanup_specs()? {
        ...
        let (failure_point, sequence, panic_contained) = match spec.executor.as_str() {
            ...
            "execute_host_panic" => (
                ...
                Some(std::panic::catch_unwind(|| panic!("gate1-v2.5 host panic fixture")).is_err()),
            ),
            "execute_cancel" => (
                None,
                vec![
...
fn synthetic_failed_output(message: String) -> Output {
    ...
    {
        let _ = message;
        unreachable!("Nexa Gate 1 v2.5 formal apparatus currently targets Unix hosts")
    }
}
...
fn formal_handshake_probe() -> Result<(), AnyError> {
    let root = repository_root().join("target/gate1-v2.5-qualification/formal-handshake");
    if root.exists() {
...
fn require_run_label(label: &str) -> Result<(), AnyError> {
    if matches!(label, "formal-run-1" | "formal-run-2" | "replay") {
        ...
        Err(format!("unauthorized Gate 1 v2.5 run label `{label}`").into())
    }
}
