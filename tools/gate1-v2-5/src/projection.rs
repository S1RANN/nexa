...
use nexa_gate1_v2_5::{AnyError, read_json, stable_value_hash, write_json};

const FIXTURE_ROOT: &str = "experiments/gate1-v2.5/fixtures/v2_4_h2";
const CHECK_ROOT: &str = "target/gate1-v2.5-dryrun";
...
pub fn h2_semantic_projection(result: &Value) -> Result<Value, AnyError> {
    ...
    let projection = json!({
        "schema_version": 1,
        "projection": "gate1-v2.5-h2-semantic-v1",
...
pub fn h2_performance_projection(result: &Value) -> Result<Value, AnyError> {
    ...
    Ok(json!({
        "schema_version": 1,
        "projection": "gate1-v2.5-h2-performance-v1",
        "warmup_samples": 100,
        "timed_samples": 1000,
...
pub fn h2_allocation_projection(result: &Value) -> Result<Value, AnyError> {
    ...
    Ok(json!({
        "schema_version": 1,
        "projection": "gate1-v2.5-h2-allocation-v1",
...
pub fn noise_invariance_check() -> Result<Value, AnyError> {
    ...
    if !all_equal(&projections) {
        failures.push("v2.4 fixtures do not have identical semantic projections".to_owned());
    }
    if !all_equal(&signatures) {
        failures.push("v2.4 fixtures do not have identical semantic signatures".to_owned());
    }
    if all_equal(&old_signatures) {
        failures.push("v2.4 contaminated signatures unexpectedly match".to_owned());
    }
    if all_equal(&performance) {
        failures.push("v2.4 performance projections unexpectedly match".to_owned());
    }
    ...
    check_result(
        ...
        &json!({
            ...
            "old_contaminated_signatures": old_signatures,
...
pub fn comparison_policy_check() -> Result<Value, AnyError> {
    ...
    *target = changed_value(target);
    ...
    if formal["status"] != "PASS" {
        failures.push("stable v2.4 Formal H2 FAIL results do not compare PASS".to_owned());
    }
    if replay["status"] != "PASS" {
        failures.push("stable v2.4 Replay H2 FAIL result does not compare PASS".to_owned());
    }
...
fn compare_performance(left: &Value, right: &Value) -> Result<Value, AnyError> {
    let policy = read_json("experiments/gate1-v2.5/h2_performance_policy.json")?;
