...
use nexa_gate1_v2_5::{AnyError, git, hash_file, read_json, stable_value_hash, write_json};
use nexa_gate1_v2_5_fixtures::{FixtureCase, artifact_bundle, expected_decision};
use serde::Deserialize;
use serde_json::{Value, json};

use crate::scenario;

const DRYRUN_ROOT: &str = "target/gate1-v2.5-dryrun";
const GRAPH_PATH: &str = "reports/history/gate1/supersession_graph.json";
const INDEX_PATH: &str = "reports/history/gate1/index.json";
const CONTRACTS_PATH: &str = "reports/contracts/gate1_v2_5_contracts.json";
...
pub fn history_check() -> Result<Value, AnyError> {
    ...
    let historical_immutability = immutable
        .iter()
        .filter(|(version, _)| version.as_str() != "gate1-v2.5")
        .all(|(_, value)| *value);
    result["metrics"] = json!({
...
pub fn governance_negative_tests() -> Result<Value, AnyError> {
    ...
    cycle
        .edges
        .push(["gate1-v2.5".to_owned(), "gate1-v1".to_owned()]);
    ...
    current_outgoing
        .edges
        .push(["gate1-v2.5".to_owned(), "gate1-v2.2".to_owned()]);
    cases.push(run_case(
    ...
    unknown_edge
        .edges
        .push(["gate1-unknown".to_owned(), "gate1-v2.5".to_owned()]);
...
pub fn h3_execution_independence() -> Result<Value, AnyError> {
    ...
    let traces = specs
        ...
        .map(|spec| serde_json::to_string(&spec.expected_operations))
        .collect::<Result<BTreeSet<_>, _>>()?;
    let formal_source = std::fs::read_to_string("tools/gate1-v2-5/src/main.rs")?;
    let forbidden = [
...
pub fn contract_satisfiability() -> Result<Value, AnyError> {
    ...
    for contract in &manifest.contracts {
        ...
        if contract.work_package == 48 {
            if contract.artifact != "__finalization__.json" || !contract.assertions.is_empty() {
                failures.push("WP-048 does not use phase-aware finalization semantics".to_owned());
            }
    ...
    let expected_wps = (1_u32..=48).collect::<BTreeSet<_>>();
    if manifest.contracts.len() != 48 || work_packages != expected_wps {
        failures.push("manifest does not cover WP1-WP48 exactly once".to_owned());
    }
    let result = json!({
        "schema_version": 1,
...
pub fn synthetic_git_chain() -> Result<Value, AnyError> {
    let root = std::env::temp_dir().join(format!(
        "nexa-gate1-v2.5-synthetic-chain-{}",
        std::process::id()
    ));
    if root.exists() {
    ...
    git_in(
        &root,
        &["config", "user.email", "gate1-v2.5@example.invalid"],
    )?;
    git_in(&root, &["config", "user.name", "Gate 1 v2.5 Dry Run"])?;
    ...
    std::fs::create_dir_all(root.join("reports/raw/gate1_v2_5"))?;
    std::fs::write(
        root.join("reports/raw/gate1_v2_5/synthetic.json"),
    ...
    let d_paths = [
        "reports/contracts/gate1_v2_5_contract_evaluation.json",
        "reports/contracts/gate1_v2_5_results_pending_finalization.json",
        "reports/gate1_v2_5_decision_pending_finalization.md",
        "reports/gate1_v2_5_summary_pending_finalization.md",
        "reports/gate1_v2_5_pilot.json",
        "reports/gate1_v2_5_budget.json",
    ];
    for path in d_paths {
    ...
    let f_paths = [
        "reports/contracts/gate1_v2_5_results.json",
        "reports/contracts/gate1_v2_5_verification_receipt.json",
        "reports/gate1_v2_5_final_decision.md",
        "reports/gate1_v2_5_summary.md",
    ...
    let finalization_paths = git_in(
        ...
        &[
            "diff",
            "--name-only",
            &format!("{decision}..{finalization}"),
        ],
    )?;
    let path_ok = evidence_paths
        .lines()
        .all(|path| path.starts_with("reports/raw/gate1_v2_5/"))
    ...
    let result = json!({
        ...
        "wp_048_pending_before_finalization": true,
        "wp_048_pass_after_candidate_finalization": recomputed,
...
pub fn prefreeze_closure() -> Result<Value, AnyError> {
    for forbidden in [
        "baseline/testing/GATE1_V2_5_AUTHORIZATION.md",
        "experiments/gate1-v2.5/authorization.json",
        "experiments/gate1-v2.5/manifest.json",
    ] {
    ...
    let frozen_root = Path::new("experiments/gate1-v2.5/prefreeze");
