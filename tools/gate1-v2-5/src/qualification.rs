...
const QUALIFICATION_ROOT: &str = "target/gate1-v2.5-qualification";
...
pub fn qualify_environment(output: &Path) -> Result<(), AnyError> {
    ...
    if status == "QUALIFIED" {
        let frozen = Path::new("experiments/gate1-v2.5/qualification");
        ...
        for name in [
            ...
            std::fs::copy(
                Path::new("target/gate1-v2.5-qualification/formal-handshake").join(name),
...
fn release_binary_probe(output: &Path) -> Result<Value, AnyError> {
    let build = run_command("cargo", &["build", "--release", "-p", "nexa-gate1-v2-5"])?;
    let binary = repository_root().join("target/release/nexa-gate1-v2-5");
...
fn qualification_markdown(qualification: &Value) -> String {
    format!(
        "# Gate 1 v2.5 Environment Qualification\n\n\
         Status: **{}**\n\n\
         Provenance protocol: `{}`\n\n\
         Atomic checks: {}\n\n\
