...
const ACCEPTANCE: &str = "baseline/testing/GATE1_ACCEPTANCE_V2_7.md";
const AUTHORIZATION: &str = "baseline/testing/GATE1_V2_7_AUTHORIZATION.md";
const AUTHORIZATION_RECORD: &str = "experiments/gate1-v2.7/authorization.json";
const THRESHOLDS: &str = "experiments/gate1-v2.7/threshold_equivalence.json";
const ENVIRONMENT: &str = "experiments/gate1-v2.7/environment.json";
const QUALIFICATION: &str = "experiments/gate1-v2.7/qualification/environment_qualification.json";
...
fn main() -> Result<(), AnyError> {
    ...
    match arguments.as_slice() {
        ...
        [command] if command == "qualify-environment" => qualification::qualify_environment(
            Path::new("target/gate1-v2.7-qualification/qualified-host"),
        ),
        ...
        [command] if command == "artifact-hygiene-check" => {
            let result =
                scenario::verify_artifact_hygiene(Path::new("target/gate1-v2.7-dryrun"))?;
            println!("{}", serde_json::to_string_pretty(&result)?);
...
fn acceptance_equivalence() -> Result<(), AnyError> {
    ...
    if weakened
        || !removed.is_empty()
        || !changed.is_empty()
        || fingerprint["experiment_version"] != "gate1-v2.7"
    ...
    {
        return Err("Gate 1 v2.7 outcome thresholds are not equivalent to v2".into());
    }
    println!(
        "{}",
...
fn evidence_static_check() -> Result<(), AnyError> {
    let roots = [
        Path::new("tools/gate1-v2-7/src"),
        Path::new("tools/gate1-v2-7-gates/src"),
        Path::new("tools/gate1-v2-7-decision/src"),
...
fn freeze() -> Result<(), AnyError> {
    ...
    let prefreeze_closure = read_json("experiments/gate1-v2.7/prefreeze/prefreeze_closure.json")?;
    if prefreeze_closure["status"] != "PASS"
        || prefreeze_closure["synthetic_artifacts_formal_evidence_usable"] != false
    {
        return Err("Gate 1 v2.7 prefreeze closure is not valid".into());
    }
    ...
    {
        return Err("Gate 1 v2.7 environment is not qualified".into());
    }
    let authorization = read_json(AUTHORIZATION_RECORD)?;
    if authorization["status"] != "AUTHORIZED"
        || authorization["prefreeze_closure"]["hash"]
            != hash_file("experiments/gate1-v2.7/prefreeze/prefreeze_closure.json")?
        ...
        || authorization["provenance_protocol"]["hash"]
            != hash_file("experiments/gate1-v2.7/prefreeze/outcome_transport.json")?
        || authorization["execution_budget"]
            != json!({"formal_run_1": 1, "formal_run_2": 1, "replay": 1, "retry": 0})
    {
        return Err("Gate 1 v2.7 authorization bindings are invalid".into());
    }
    let contract_manifest = read_json("reports/contracts/gate1_v2_7_contracts.json")?;
    if contract_manifest["contracts"]
        .as_array()
        .map_or(0, Vec::len)
        != 50
    {
        return Err("Gate 1 v2.7 Contract Manifest does not cover 50 work packages".into());
    }
    ...
    let runner_hash = stable_value_hash(&json!({
        "runner_lib": hashes["tools/gate1-v2-7/src/lib.rs"],
        "runner_main": hashes["tools/gate1-v2-7/src/main.rs"],
        "gates_main": hashes["tools/gate1-v2-7-gates/src/main.rs"],
        "gates_lib": hashes["tools/gate1-v2-7-gates/src/lib.rs"],
        "decision": hashes["tools/gate1-v2-7-decision/src/main.rs"],
        ...
        ,"qualification": hashes["tools/gate1-v2-7/src/qualification.rs"]
        ,"prefreeze": hashes["tools/gate1-v2-7/src/prefreeze.rs"]
        ,"scenario": hashes["tools/gate1-v2-7/src/scenario.rs"]
        ,"projection": hashes["tools/gate1-v2-7/src/projection.rs"]
        ,"history_v2_5": hashes["tools/gate1-v2-7/src/history_v2_5.rs"]
        ,"history_v2_6": hashes["tools/gate1-v2-7/src/history_v2_6.rs"]
        ,"fixtures_lib": hashes["tools/gate1-v2-7-fixtures/src/lib.rs"]
        ,"fixtures_main": hashes["tools/gate1-v2-7-fixtures/src/main.rs"]
    }));
    let manifest = json!({
        "schema_version": 3,
        "experiment_version": "gate1-v2.7",
        ...
        "prefreeze_closure_hash": hashes["experiments/gate1-v2.7/prefreeze/prefreeze_closure.json"],
        "supersession_graph_hash": hashes["reports/history/gate1/supersession_graph.json"],
        "sample_hash": sample_hash,
        "sample_hashes": sample_hashes,
        "runner_hash": runner_hash,
        "worker_hash": hashes["tools/gate1-v2-7/src/main.rs"],
        "decision_tool_hash": hashes["tools/gate1-v2-7-decision/src/main.rs"],
        "receipt_generator_hash": hashes["tools/gate1-v2-7-decision/src/main.rs"],
        "h2_benchmark_config_hash": hashes["experiments/gate1/benchmark.json"],
        "contract_manifest_hash": hashes["reports/contracts/gate1_v2_7_contracts.json"],
        "bound_hashes": hashes
    });
    write_json(Path::new(MANIFEST), &manifest)?;
    println!(
        "froze Gate 1 v2.7 manifest with {} bound inputs",
        paths.len()
    );
    Ok(())
}
...
fn bound_input_paths() -> Vec<String> {
    let mut paths = [
        ...
        ENVIRONMENT,
        QUALIFICATION,
        "experiments/gate1-v2.7/qualification/environment_qualification.md",
        "experiments/gate1-v2.7/qualification/environment_qualification_hashes.json",
        "experiments/gate1-v2.7/qualification/root-cause.json",
        "experiments/gate1-v2.7/qualification/formal-handshake/process_attestation.json",
        "experiments/gate1-v2.7/qualification/formal-handshake/parent_verification.json",
        "experiments/gate1-v2.7/qualification/formal-handshake/probe.json",
        "experiments/gate1-v2.7/prefreeze/history_check.json",
        "experiments/gate1-v2.7/prefreeze/governance_negative_tests.json",
        "experiments/gate1-v2.7/prefreeze/status_lint.json",
        "experiments/gate1-v2.7/prefreeze/scenario_independence.json",
        "experiments/gate1-v2.7/prefreeze/outcome_transport.json",
        "experiments/gate1-v2.7/prefreeze/h1_transformer_equivalence.json",
        "experiments/gate1-v2.7/prefreeze/h2_dimension_effectiveness.json",
        "experiments/gate1-v2.7/prefreeze/h2_cleanup_independence.json",
        "experiments/gate1-v2.7/prefreeze/h3_execution_independence.json",
        "experiments/gate1-v2.7/prefreeze/raw_regeneration_exercise.json",
        "experiments/gate1-v2.7/prefreeze/contract_satisfiability.json",
        "experiments/gate1-v2.7/prefreeze/decision_branches.json",
        "experiments/gate1-v2.7/prefreeze/terminal_short_circuit.json",
        "experiments/gate1-v2.7/prefreeze/synthetic_git_chain.json",
        "experiments/gate1-v2.7/prefreeze/structural_failure_regression.json",
        "experiments/gate1-v2.7/prefreeze/h2_projection.json",
        "experiments/gate1-v2.7/prefreeze/h2_noise_invariance.json",
        "experiments/gate1-v2.7/prefreeze/h2_semantic_sensitivity.json",
        "experiments/gate1-v2.7/prefreeze/comparison_policy.json",
        "experiments/gate1-v2.7/prefreeze/comparison_truth_table.json",
        "experiments/gate1-v2.7/prefreeze/inconclusive_contract.json",
        "experiments/gate1-v2.7/prefreeze/prefreeze_closure.json",
        "experiments/gate1-v2.7/h2_semantic_projection.json",
        "experiments/gate1-v2.7/h2_performance_policy.json",
        "experiments/gate1-v2.7/decision_state_machine.json",
        "experiments/gate1-v2.7/fixtures/v2_5_h2/formal-run-1.json",
        "experiments/gate1-v2.7/fixtures/v2_5_h2/formal-run-2.json",
        "experiments/gate1-v2.7/fixtures/v2_5_h2/replay.json",
        "experiments/gate1-v2.7/fixtures/v2_5_comparisons/formal.json",
        "experiments/gate1-v2.7/fixtures/v2_5_comparisons/replay.json",
        ...
        "tools/gate1-v2-7/src/lib.rs",
        "tools/gate1-v2-7/src/main.rs",
        "tools/gate1-v2-7/src/history_v2_5.rs",
        "tools/gate1-v2-7/src/history_v2_6.rs",
        "tools/gate1-v2-7/src/projection.rs",
        "tools/gate1-v2-7/src/qualification.rs",
        "tools/gate1-v2-7/src/prefreeze.rs",
        "tools/gate1-v2-7/src/scenario.rs",
        "tools/gate1-v2-7-fixtures/src/lib.rs",
        "tools/gate1-v2-7-fixtures/src/main.rs",
        "tools/gate1-v2-7-gates/src/main.rs",
        "tools/gate1-v2-7-gates/src/lib.rs",
        "tools/gate1-v2-7-decision/src/main.rs",
        ...
        "reports/contracts/gate1_v2_7_contracts.json",
    ]
    .into_iter()
    .map(str::to_owned)
    .collect::<Vec<_>>();
...
fn supervisor(label: &str) -> Result<(), AnyError> {
    ...
    if result["apparatus_status"] != "PASS" {
        return Err(format!("Gate 1 v2.7 {label} was invalid: {}", result["failures"]).into());
    }
    println!(
        "Gate 1 v2.7 {label}: apparatus PASS, outcomes {}",
...
fn run_h1_mutation(
    ...
    let hand_build = cargo_check_binding(
        ...
        &hand_file,
        &repository_root()
            .join("target/gate1-v2.7-build/h1")
            .join(format!("{id}-hand")),
        false,
    )?;
        ...
        Ok(changed_idl) => {
            ...
            if build.status.success() {
                ...
                let module = nexa_compiler::compile_with_metadata(
                    ...
                    changed_hash,
                    StableId::from_name("gate1-v2.7-h1-schema"),
                )?;
...
fn h2_worker(label: &str, supplied_nonce: &str) -> Result<(), AnyError> {
    ...
    let performance_pass = timing_failures.is_empty()
        ...
        && benchmark_processes.iter().all(|process| {
            process["warmup"].as_u64() == Some(100) && process["samples"].as_u64() == Some(1000)
        });
    reserve_result_path(&root, label)?;
    let process = recorder.finish()?;
    events.sync()?;
    ...
    for spec in scenario::h2_cleanup_specs()? {
        ...
        let (failure_point, sequence, panic_contained) = match spec.executor.as_str() {
            ...
            "execute_host_panic" => (
                ...
                Some(std::panic::catch_unwind(|| panic!("gate1-v2.7 host panic fixture")).is_err()),
            ),
            "execute_cancel" => (
                None,
                vec![
...
fn synthetic_failed_output(message: String) -> Output {
    ...
    {
        let _ = message;
        unreachable!("Nexa Gate 1 v2.7 formal apparatus currently targets Unix hosts")
    }
}
...
fn formal_handshake_probe() -> Result<(), AnyError> {
    let root = repository_root().join("target/gate1-v2.7-qualification/formal-handshake");
    if root.exists() {
...
fn require_run_label(label: &str) -> Result<(), AnyError> {
    if matches!(label, "formal-run-1" | "formal-run-2" | "replay") {
        ...
        Err(format!("unauthorized Gate 1 v2.7 run label `{label}`").into())
    }
}
