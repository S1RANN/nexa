...
use nexa_gate1_v2_7_gates::{GATE_NAMES, RAW_ROOT, gate_hashes, generate_from_raw};
use serde::Deserialize;
use serde_json::{Value, json};

const CONTRACTS: &str = "reports/contracts/gate1_v2_7_contracts.json";
const EVALUATION: &str = "reports/contracts/gate1_v2_7_contract_evaluation.json";
const PENDING_RESULTS: &str = "reports/contracts/gate1_v2_7_results_pending_receipt.json";
const PENDING_DECISION: &str = "reports/gate1_v2_7_decision_pending_receipt.md";
const PENDING_SUMMARY: &str = "reports/gate1_v2_7_summary_pending_receipt.md";
const PILOT_REPORT: &str = "reports/gate1_v2_7_pilot.json";
const BUDGET_REPORT: &str = "reports/gate1_v2_7_budget.json";
const FINAL_RESULTS: &str = "reports/contracts/gate1_v2_7_results.json";
const FINALIZATION: &str = "reports/contracts/gate1_v2_7_finalization.json";
const RECEIPT: &str = "reports/contracts/gate1_v2_7_verification_receipt.json";
const FINAL_DECISION: &str = "reports/gate1_v2_7_final_decision.md";
const FINAL_SUMMARY: &str = "reports/gate1_v2_7_summary.md";
const GATES: &str = "reports/raw/gate1_v2_7/gates";
const CURRENT_STATUS: &str = "reports/history/gate1/current_status.json";
const STATUS_START: &str = "<!-- gate1-v2.7-status:start -->";
const STATUS_END: &str = "<!-- gate1-v2.7-status:end -->";
...
fn main() -> Result<(), AnyError> {
    ...
    match std::env::args().skip(1).collect::<Vec<_>>().as_slice() {
        ...
        _ => Err("usage: nexa-gate1-v2-7-decision decision-state-check|generate-pending|verify-pending|generate-receipt|verify-receipt|finalize|verify-final|status-lint".into()),
    }
}
...
fn decision_state_check() -> Result<(), AnyError> {
    ...
    write_json(
        Path::new("target/gate1-v2.7-dryrun/decision_state_check.json"),
        &result,
    )?;
    if status != "PASS" {
...
fn generate_pending() -> Result<(), AnyError> {
    ensure_head_phase("E2.7", validate_e_paths)?;
    ...
    println!("Gate 1 v2.7 pending decision: {decision}");
    Ok(())
}
...
fn verify_pending() -> Result<(), AnyError> {
    ...
    let reconstruction = reconstruct(Path::new(GATES), false)?;
    verify_pending_files(&reconstruction)?;
    println!("Gate 1 v2.7 D2.7 candidate files verified");
    Ok(())
}

fn finalize() -> Result<(), AnyError> {
    ensure_head_phase("R2.7", validate_r_paths)?;
    ...
    {
        return Err("D2.7 pending result is not finalizable after Receipt".into());
    }
    ...
    println!("Gate 1 v2.7 deterministic F2.7 candidate tree generated from R2.7 Receipt");
    Ok(())
}

fn generate_receipt() -> Result<(), AnyError> {
    ensure_head_phase("D2.7", validate_d_paths)?;
    let reconstruction = reconstruct(Path::new(GATES), false)?;
    if !reconstruction.gaps.is_empty() {
    ...
    println!("Gate 1 v2.7 R2.7 Receipt candidate generated");
    Ok(())
}
...
fn verify_receipt() -> Result<(), AnyError> {
    ...
    verify_receipt_candidate(&reconstruction, &receipt, &files)?;
    println!("Gate 1 v2.7 R2.7 Receipt candidate verified");
    Ok(())
}
...
fn verify_final() -> Result<(), AnyError> {
    ...
    {
        return Err("F2.7 Receipt does not bind the actual I/E/D chain".into());
    }
    ...
    if paths
        != F_PATHS
            ...
            .collect::<BTreeSet<_>>()
    {
        return Err(format!("F2.7 path set is invalid: {paths:?}").into());
    }
    validate_r_paths(&r)?;
    validate_e_paths(&e)?;
    validate_d_paths(&d)?;
    ...
    println!("Gate 1 v2.7 F2.7 and I/E/D/R/F topology: VERIFIED");
    Ok(())
}
...
fn structured_pivot_approval(failed_hypotheses: &[String]) -> Result<bool, AnyError> {
    let path = Path::new("reports/gate1_v2_7_pivot_approval.json");
    if !path.exists() {
        return Ok(false);
    }
...
fn evaluation_document(reconstruction: &Reconstruction, final_phase: bool) -> Value {
    json!({
        "schema_version": 1,
        "experiment_version": "gate1-v2.7",
        "phase": if final_phase {"F2.7_CANDIDATE"} else {"D2.7"},
        "contract_count": 50,
...
fn render_pending_decision(results: &Value) -> String {
    format!(
        "# Gate 1 v2.7 Decision Pending Receipt\n\nProduct decision: **{}**\n\nGate 1 status: **DECISION_COMPUTED_PENDING_RECEIPT**\n\nMilestone 5.0R7: **INCOMPLETE**\n\nWP-050 remains PENDING_FINALIZATION until the Receipt is committed and F2.7 matches its eight expected hashes.\n",
...
fn final_document_blocks(decision: &str) -> BTreeMap<&'static str, String> {
    BTreeMap::from([
        (
            "README.md",
            format!(
                "{STATUS_START}\nGate 1 v2.4: STRUCTURAL_CLOSURE_FAILED / NOT AUTHORIZED FOR DECISION\nGate 1 v2.5: STRUCTURAL_CLOSURE_FAILED / NOT AUTHORIZED FOR DECISION\nGate 1 v2.6: STRUCTURAL_CLOSURE_FAILED / INCOMPLETE / RECORDED STOP NOT AUTHORIZED\nGate 1 v2.7: VERIFIED_TERMINAL_DECISION\nCurrent decision: {decision}\nMilestone 5.0R7: COMPLETE\nPush: AUTHORIZED\n{STATUS_END}"
            ),
        ),
        (
            "ROADMAP.md",
            format!(
                "{STATUS_START}\nCurrent project gate: **Gate 1 v2.7 verified terminal decision**.\n\nGate 1 v2.5 is **STRUCTURAL_CLOSURE_FAILED** and not decision-usable. Gate 1 v2.6 is **STRUCTURAL_CLOSURE_FAILED / INCOMPLETE**; its recorded STOP is not authorized.\n\nCurrent Gate 1 v2.7 decision: **{decision}**.\n\nCurrent milestone: **5.0R7 — COMPLETE**.\n{STATUS_END}"
            ),
        ),
        (
            "baseline/BASELINE_INDEX.md",
            format!(
                "{STATUS_START}\nGate 1 v2.5 is **STRUCTURAL_CLOSURE_FAILED** and not decision-usable. Gate 1 v2.6 is **STRUCTURAL_CLOSURE_FAILED / INCOMPLETE** and its recorded STOP is unauthorized. Gate 1 v2.7 is **VERIFIED_TERMINAL_DECISION**, its decision is **{decision}**, and Milestone 5.0R7 is **COMPLETE**.\n{STATUS_END}"
            ),
        ),
    ])
}
...
fn final_roadmap_rows(decision: &str) -> [String; 2] {
    [
        "| Gate 1 v2.7 | Stable semantic projection and explicit finalization | I/E/D/R/F chain and Raw-derived Receipt | Verified terminal decision |".to_owned(),
        format!(
            "| Gate 1 v2.7 Decision | Valid v2.7 evidence and 50/50 contracts | Verified I/E/D/R/F and empty structural gaps | {decision} |"
        ),
    ]
}
...
fn build_receipt(
    ...
    let receipt = json!({
        ...
        "topology": "I2.7 -> E2.7 -> D2.7 -> R2.7 -> F2.7",
        ...
        "expected_f_parent": "R2.7_RECEIPT_COMMIT",
        ...
        "known_structural_gaps": reconstruction.gaps,
        "frozen_manifest_hash": hash_file("experiments/gate1-v2.7/manifest.json")?,
...
fn finalization_files(
    ...
    let roadmap = replace_table_row(&roadmap, "| Gate 1 v2.7 |", &rows[0])?;
    let roadmap = replace_table_row(&roadmap, "| Gate 1 v2.7 Decision |", &rows[1])?;
...
fn verify_receipt_candidate(
    ...
    if receipt["status"] != "RECEIPT_VERIFIED_PENDING_F"
        || receipt["topology"] != "I2.7 -> E2.7 -> D2.7 -> R2.7 -> F2.7"
        ...
        || receipt["known_structural_gaps"]
            ...
            .is_none_or(|gaps| !gaps.is_empty())
    {
        return Err("R2.7 Receipt candidate is incomplete".into());
    }
    verify_expected_finalization_hashes(files, receipt)
}
...
fn verify_pending_files(reconstruction: &Reconstruction) -> Result<(), AnyError> {
    ...
    {
        return Err("D2.7 candidate files differ from reconstruction".into());
    }
    Ok(())
}
...
fn verify_candidate_files(reconstruction: &Reconstruction) -> Result<(), AnyError> {
    ...
    for (path, expected) in &files {
        if std::fs::read(path)? != *expected {
            return Err(format!("F2.7 file {path} differs from reconstruction").into());
        }
    }
    let receipt = read_json(RECEIPT)?;
    verify_receipt_candidate(reconstruction, &receipt, &files)
...
fn status_lint() -> Result<(), AnyError> {
    ...
    if status["current_experiment"] != "gate1-v2.7" {
        return Err("current status does not point at gate1-v2.7".into());
    }
    ...
    println!("Gate 1 v2.7 status lint: PASS");
    Ok(())
}
...
fn validate_e_paths(commit: &str) -> Result<(), AnyError> {
    ...
    if paths.is_empty()
        || paths
            .iter()
            .any(|path| !path.starts_with("reports/raw/gate1_v2_7/"))
    {
        return Err(format!("E2.7 path set is invalid: {paths:?}").into());
    }
    Ok(())
}
...
fn validate_d_paths(commit: &str) -> Result<(), AnyError> {
    ...
    let expected = D_PATHS
        ...
        .collect::<BTreeSet<_>>();
    if paths != expected {
        return Err(format!("D2.7 path set is invalid: {paths:?}").into());
    }
    Ok(())
}
...
fn validate_r_paths(commit: &str) -> Result<(), AnyError> {
    ...
    let expected = R_PATHS
        ...
        .collect::<BTreeSet<_>>();
    if paths != expected {
        return Err(format!("R2.7 path set is invalid: {paths:?}").into());
    }
    Ok(())
}
...
fn receipt_temp_directory() -> PathBuf {
    Path::new("target").join(format!(
        "gate1-v2.7-receipt-regeneration-{}",
        std::process::id()
    ))
}
...
fn replace_block_text(source: &str, replacement: &str) -> Result<String, AnyError> {
    let start = source
        .find(STATUS_START)
        .ok_or("v2.7 status block start is missing")?;
    let end = source
        .find(STATUS_END)
        .ok_or("v2.7 status block end is missing")?
        + STATUS_END.len();
