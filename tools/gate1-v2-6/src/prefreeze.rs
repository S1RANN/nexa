...
use nexa_gate1_v2_6_fixtures::{FixtureCase, artifact_bundle};
use serde::Deserialize;
use serde_json::{Value, json};

use crate::scenario;

const DRYRUN_ROOT: &str = "target/gate1-v2.6-dryrun";
const GRAPH_PATH: &str = "reports/history/gate1/supersession_graph.json";
const INDEX_PATH: &str = "reports/history/gate1/index.json";
const CONTRACTS_PATH: &str = "reports/contracts/gate1_v2_6_contracts.json";
...
pub fn history_check() -> Result<Value, AnyError> {
    ...
    let historical_immutability = immutable
        .iter()
        .filter(|(version, _)| version.as_str() != "gate1-v2.6")
        .all(|(_, value)| *value);
    result["metrics"] = json!({
...
pub fn governance_negative_tests() -> Result<Value, AnyError> {
    ...
    cycle
        .edges
        .push(["gate1-v2.6".to_owned(), "gate1-v1".to_owned()]);
    ...
    current_outgoing
        .edges
        .push(["gate1-v2.6".to_owned(), "gate1-v2.2".to_owned()]);
    cases.push(run_case(
    ...
    unknown_edge
        .edges
        .push(["gate1-unknown".to_owned(), "gate1-v2.6".to_owned()]);
...
pub fn h3_execution_independence() -> Result<Value, AnyError> {
    ...
    let traces = specs
        ...
        .map(|spec| serde_json::to_string(&spec.expected_operations))
        .collect::<Result<BTreeSet<_>, _>>()?;
    let formal_source = std::fs::read_to_string("tools/gate1-v2-6/src/main.rs")?;
    let forbidden = [
...
pub fn synthetic_git_chain() -> Result<Value, AnyError> {
    let root = std::env::temp_dir().join(format!(
        "nexa-gate1-v2.6-synthetic-chain-{}",
        std::process::id()
    ));
    if root.exists() {
    ...
    git_in(
        &root,
        &["config", "user.email", "gate1-v2.6@example.invalid"],
    )?;
    git_in(&root, &["config", "user.name", "Gate 1 v2.6 Dry Run"])?;
    ...
    std::fs::create_dir_all(root.join("reports/raw/gate1_v2_6"))?;
    std::fs::write(
        root.join("reports/raw/gate1_v2_6/synthetic.json"),
    ...
    let d_paths = [
        "reports/contracts/gate1_v2_6_contract_evaluation.json",
        "reports/contracts/gate1_v2_6_results_pending_receipt.json",
        "reports/gate1_v2_6_decision_pending_receipt.md",
        "reports/gate1_v2_6_summary_pending_receipt.md",
        "reports/gate1_v2_6_pilot.json",
        "reports/gate1_v2_6_budget.json",
    ];
    for path in d_paths {
    ...
    let f_paths = [
        "reports/contracts/gate1_v2_6_results.json",
        "reports/contracts/gate1_v2_6_finalization.json",
        "reports/gate1_v2_6_final_decision.md",
        "reports/gate1_v2_6_summary.md",
    ...
    let expected_hashes = final_bytes
        ...
        .collect::<BTreeMap<_, _>>();
    let receipt_path = "reports/contracts/gate1_v2_6_verification_receipt.json";
    ...
    let path_ok = evidence_paths
        .lines()
        .all(|path| path.starts_with("reports/raw/gate1_v2_6/"))
...
pub fn prefreeze_closure() -> Result<Value, AnyError> {
    for forbidden in [
        "baseline/testing/GATE1_V2_6_AUTHORIZATION.md",
        "experiments/gate1-v2.6/authorization.json",
        "experiments/gate1-v2.6/manifest.json",
    ] {
    ...
    let qualification =
        read_json("experiments/gate1-v2.6/qualification/environment_qualification.json")?;
    ...
    let frozen_root = Path::new("experiments/gate1-v2.6/prefreeze");
