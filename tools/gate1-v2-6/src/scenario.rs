...
pub const H1_MANIFEST: &str = "experiments/gate1-v2.6/h1_mutations.json";
pub const H1_HANDWRITTEN: &str = "experiments/gate1-v2.6/fixtures/h1_handwritten.rs";
pub const H2_MANIFEST: &str = "experiments/gate1-v2.6/h2_matrix.json";
pub const H2_CLEANUP_MANIFEST: &str = "experiments/gate1-v2.6/h2_cleanup_matrix.json";
pub const H3_MANIFEST: &str = "experiments/gate1-v2.6/h3_scenarios.json";
pub const SCENARIO_SCHEMA: &str = "experiments/gate1-v2.6/scenario_schema.json";
...
pub fn status_lint() -> Result<Value, AnyError> {
    ...
    let negative_cases = [
        (
            ...
            readme.clone(),
            roadmap.replace(
                &format!("| Gate 1 v2.6 |{}", expected.roadmap_row_tail),
                "| Gate 1 v2.6 | scenario | decision | Complete",
            ),
            baseline.clone(),
        ),
        (
...
fn status_failures(
    ...
    for (path, source) in [
        ...
        if !source.contains("gate1-v2.6-status:start") || !source.contains("gate1-v2.6-status:end")
        {
    ...
    if !roadmap.contains(&format!("| Gate 1 v2.6 |{}", expected.roadmap_row_tail)) {
    ...
    if expected.experiment != "gate1-v2.6" {
        failures.push("current experiment is not gate1-v2.6".to_owned());
    }
    failures
}
