...
const ACCEPTANCE: &str = "baseline/testing/GATE1_ACCEPTANCE_V2_9.md";
const AUTHORIZATION: &str = "baseline/testing/GATE1_V2_9_AUTHORIZATION.md";
const AUTHORIZATION_RECORD: &str = "experiments/gate1-v2.9/authorization.json";
const THRESHOLDS: &str = "experiments/gate1-v2.9/threshold_equivalence.json";
const ENVIRONMENT: &str = "experiments/gate1-v2.9/environment.json";
const QUALIFICATION: &str = "experiments/gate1-v2.9/qualification/environment_qualification.json";
...
fn main() -> Result<(), AnyError> {
    ...
    match arguments.as_slice() {
        ...
        [command] if command == "qualify-environment" => qualification::qualify_environment(
            Path::new("target/gate1-v2.9-qualification/qualified-host"),
        ),
        ...
        [command] if command == "artifact-hygiene-check" => {
            let result =
                scenario::verify_artifact_hygiene(Path::new("target/gate1-v2.9-dryrun"))?;
            println!("{}", serde_json::to_string_pretty(&result)?);
...
fn acceptance_equivalence() -> Result<(), AnyError> {
    ...
    if weakened
        || !removed.is_empty()
        || !changed.is_empty()
        || fingerprint["experiment_version"] != "gate1-v2.9"
        ...
        || fingerprint["authorized_apparatus_changes"]
            != json!([
                ...
                "historical v2.8 decision-semantic invalidation",
                "per-run aggregate hypothesis outcome reconstruction",
                "formal-data stable core failure and decision assertions"
            ])
    {
        return Err("Gate 1 v2.9 outcome thresholds are not equivalent to v2".into());
    }
    println!(
        "{}",
...
fn evidence_static_check() -> Result<(), AnyError> {
    let roots = [
        Path::new("tools/gate1-v2-9/src"),
        Path::new("tools/gate1-v2-9-gates/src"),
        Path::new("tools/gate1-v2-9-decision/src"),
...
fn freeze() -> Result<(), AnyError> {
    if Path::new(MANIFEST).exists() {
        return Err("Gate 1 v2.9 manifest already exists; frozen inputs are immutable".into());
    }
    acceptance_equivalence()?;
    evidence_static_check()?;
    let history = prefreeze::history_check()?;
    ensure_pass(&history, "history")?;
    let prefreeze_closure = read_json("experiments/gate1-v2.9/prefreeze/prefreeze_closure.json")?;
    if prefreeze_closure["status"] != "PASS"
        || prefreeze_closure["synthetic_artifacts_formal_evidence_usable"] != false
    {
        return Err("Gate 1 v2.9 prefreeze closure is not valid".into());
    }
    ...
    {
        return Err("Gate 1 v2.9 environment is not qualified".into());
    }
    let authorization = read_json(AUTHORIZATION_RECORD)?;
    if authorization["schema_version"] != 1
        || authorization["experiment_version"] != "gate1-v2.9"
        || authorization["status"] != "AUTHORIZED"
        || authorization["approved_by"] != "repository owner"
        || authorization["previous_experiment"]["experiment_version"] != "gate1-v2.8"
        || authorization["previous_experiment"]["status"] != "SEMANTICALLY_INSUFFICIENT"
        || authorization["previous_experiment"]["formal_evidence_usable"] != true
        || authorization["previous_experiment"]["recorded_product_decision_authorized"] != false
        || authorization["previous_experiment"]["correct_stable_core_failures"]
            != json!(["H1", "H2", "H3"])
        || authorization["previous_experiment"]["correct_product_decision"] != "STOP"
        || authorization["previous_experiment"]["evidence_imported_into_v2_9"] != false
        || authorization["prefreeze_closure"]["path"]
            != "experiments/gate1-v2.9/prefreeze/prefreeze_closure.json"
        || authorization["prefreeze_closure"]["hash"]
            != hash_file("experiments/gate1-v2.9/prefreeze/prefreeze_closure.json")?
        || authorization["prefreeze_closure"]["status"] != "PASS"
        || authorization["environment_qualification"]["path"] != QUALIFICATION
        || authorization["environment_qualification"]["hash"] != hash_file(QUALIFICATION)?
        || authorization["environment_qualification"]["status"] != "QUALIFIED"
        || authorization["acceptance_equivalence"]["path"] != THRESHOLDS
        || authorization["acceptance_equivalence"]["hash"] != hash_file(THRESHOLDS)?
        || authorization["acceptance_equivalence"]["status"] != "PASS"
        ...
        || authorization["provenance_protocol"]["hash"]
            != hash_file("experiments/gate1-v2.9/prefreeze/outcome_transport.json")?
        || authorization["execution_budget"]
            != json!({"formal_run_1": 1, "formal_run_2": 1, "replay": 1, "retry": 0})
        || authorization["outcome_thresholds"] != "UNCHANGED_FROM_GATE1_V2_8"
        || authorization["authorized_apparatus_changes"]
            != json!([
                "historical v2.8 decision-semantic invalidation",
                "per-run aggregate hypothesis outcome reconstruction",
                "stable semantic signature and formal comparison validation",
                "formal-data WP24 WP25 WP26 WP44 WP47 assertions",
                "independent Raw-to-decision cross-check",
                "I2.9 E2.9 D2.9 R2.9 F2.9 finalization protocol"
            ])
        || authorization["production_feature_changes_authorized"] != false
    {
        return Err("Gate 1 v2.9 authorization bindings are invalid".into());
    }
    let contract_manifest = read_json("reports/contracts/gate1_v2_9_contracts.json")?;
    if contract_manifest["contracts"]
        .as_array()
        .map_or(0, Vec::len)
        != 50
    {
        return Err("Gate 1 v2.9 Contract Manifest does not cover 50 work packages".into());
    }
    ...
    let runner_hash = stable_value_hash(&json!({
        "runner_lib": hashes["tools/gate1-v2-9/src/lib.rs"],
        "runner_main": hashes["tools/gate1-v2-9/src/main.rs"],
        "gates_main": hashes["tools/gate1-v2-9-gates/src/main.rs"],
        "gates_lib": hashes["tools/gate1-v2-9-gates/src/lib.rs"],
        "decision": hashes["tools/gate1-v2-9-decision/src/main.rs"],
        ...
        ,"qualification": hashes["tools/gate1-v2-9/src/qualification.rs"]
        ,"prefreeze": hashes["tools/gate1-v2-9/src/prefreeze.rs"]
        ,"scenario": hashes["tools/gate1-v2-9/src/scenario.rs"]
        ,"projection": hashes["tools/gate1-v2-9/src/projection.rs"]
        ,"history_v2_5": hashes["tools/gate1-v2-9/src/history_v2_5.rs"]
        ,"history_v2_6": hashes["tools/gate1-v2-9/src/history_v2_6.rs"]
        ,"fixtures_lib": hashes["tools/gate1-v2-9-fixtures/src/lib.rs"]
        ,"fixtures_main": hashes["tools/gate1-v2-9-fixtures/src/main.rs"]
    }));
    let manifest = json!({
        "schema_version": 3,
        "experiment_version": "gate1-v2.9",
        ...
        "prefreeze_closure_hash": hashes["experiments/gate1-v2.9/prefreeze/prefreeze_closure.json"],
        "supersession_graph_hash": hashes["reports/history/gate1/supersession_graph.json"],
        "sample_hash": sample_hash,
        "sample_hashes": sample_hashes,
        "runner_hash": runner_hash,
        "worker_hash": hashes["tools/gate1-v2-9/src/main.rs"],
        "decision_tool_hash": hashes["tools/gate1-v2-9-decision/src/main.rs"],
        "receipt_generator_hash": hashes["tools/gate1-v2-9-decision/src/main.rs"],
        "h2_benchmark_config_hash": hashes["experiments/gate1/benchmark.json"],
        "contract_manifest_hash": hashes["reports/contracts/gate1_v2_9_contracts.json"],
        "bound_hashes": hashes
    });
    write_json(Path::new(MANIFEST), &manifest)?;
    println!(
        "froze Gate 1 v2.9 manifest with {} bound inputs",
        paths.len()
    );
    Ok(())
}
...
fn bound_input_paths() -> Vec<String> {
    let mut paths = [
        ...
        ENVIRONMENT,
        QUALIFICATION,
        "experiments/gate1-v2.9/qualification/environment_qualification.md",
        ...
        "experiments/gate1-v2.9/fixtures/v2_5_h2/formal-run-1.json",
        "experiments/gate1-v2.9/fixtures/v2_5_h2/formal-run-2.json",
        "experiments/gate1-v2.9/fixtures/v2_5_h2/replay.json",
        "experiments/gate1-v2.9/fixtures/v2_5_comparisons/formal.json",
        "experiments/gate1-v2.9/fixtures/v2_5_comparisons/replay.json",
        ...
        "tools/gate1-v2-9/src/lib.rs",
        "tools/gate1-v2-9/src/main.rs",
        "tools/gate1-v2-9/src/history_v2_5.rs",
        "tools/gate1-v2-9/src/history_v2_6.rs",
        "tools/gate1-v2-9/src/projection.rs",
        "tools/gate1-v2-9/src/qualification.rs",
        "tools/gate1-v2-9/src/prefreeze.rs",
        "tools/gate1-v2-9/src/scenario.rs",
        "tools/gate1-v2-9-fixtures/src/lib.rs",
        "tools/gate1-v2-9-fixtures/src/main.rs",
        "tools/gate1-v2-9-gates/src/main.rs",
...
fn supervisor(label: &str) -> Result<(), AnyError> {
    ...
    if result["apparatus_status"] != "PASS" {
        return Err(format!("Gate 1 v2.9 {label} was invalid: {}", result["failures"]).into());
    }
    println!(
        "Gate 1 v2.9 {label}: apparatus PASS, outcomes {}",
...
fn run_h1_mutation(
    ...
    let hand_build = cargo_check_binding(
        ...
        &hand_file,
        &repository_root()
            .join("target/gate1-v2.9-build/h1")
            .join(format!("{id}-hand")),
        false,
    )?;
        ...
        Ok(changed_idl) => {
            ...
            if build.status.success() {
                ...
                let module = nexa_compiler::compile_with_metadata(
                    ...
                    changed_hash,
                    StableId::from_name("gate1-v2.9-h1-schema"),
                )?;
    ...
    for spec in scenario::h2_cleanup_specs()? {
        ...
        let (failure_point, sequence, panic_contained) = match spec.executor.as_str() {
            ...
            "execute_host_panic" => (
                ...
                Some(std::panic::catch_unwind(|| panic!("gate1-v2.9 host panic fixture")).is_err()),
            ),
            "execute_cancel" => (
                None,
                vec![
...
fn synthetic_failed_output(message: String) -> Output {
    ...
    {
        let _ = message;
        unreachable!("Nexa Gate 1 v2.9 formal apparatus currently targets Unix hosts")
    }
}
...
fn formal_handshake_probe() -> Result<(), AnyError> {
    let root = repository_root().join("target/gate1-v2.9-qualification/formal-handshake");
    if root.exists() {
...
fn require_run_label(label: &str) -> Result<(), AnyError> {
    if matches!(label, "formal-run-1" | "formal-run-2" | "replay") {
        ...
        Err(format!("unauthorized Gate 1 v2.9 run label `{label}`").into())
    }
}
