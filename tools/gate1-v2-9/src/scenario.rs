...
pub const H1_MANIFEST: &str = "experiments/gate1-v2.9/h1_mutations.json";
pub const H1_HANDWRITTEN: &str = "experiments/gate1-v2.9/fixtures/h1_handwritten.rs";
pub const H2_MANIFEST: &str = "experiments/gate1-v2.9/h2_matrix.json";
pub const H2_CLEANUP_MANIFEST: &str = "experiments/gate1-v2.9/h2_cleanup_matrix.json";
pub const H3_MANIFEST: &str = "experiments/gate1-v2.9/h3_scenarios.json";
pub const SCENARIO_SCHEMA: &str = "experiments/gate1-v2.9/scenario_schema.json";
...
pub fn status_lint() -> Result<Value, AnyError> {
    ...
    if registry["current_experiment"] == "gate1-v2.8" && graph["current"] == "gate1-v2.9" {
        let failures = prefinal_publication_failures(&registry, &readme, &roadmap, &baseline);
        let negative_results = [
            (
                "early-v2.9-readme-publication",
                readme.replace(
            ...
            (
                "early-v2.9-roadmap-publication",
                readme.clone(),
                roadmap.replace(
                    "<!-- gate1-v2.8-status:start -->",
                    "<!-- gate1-v2.9-status:start -->",
            ...
            (
                "early-v2.9-baseline-publication",
                readme.clone(),
                roadmap.clone(),
                baseline.replace(
                    "<!-- gate1-v2.8-status:start -->",
        ...
        .map(|(name, r, m, b)| {
            json!({
                "name": name,
                "detected": !prefinal_publication_failures(&registry, &r, &m, &b).is_empty()
            })
        });
        let status = if failures.is_empty()
            && negative_results.iter().all(|case| case["detected"] == true)
        {
            "PASS"
        } else {
            "FAIL"
        ...
        let result = json!({
            "schema_version": 1,
            "registry": registry,
            "supersession_current": graph["current"],
            "publication_deferred_to": "F2.9",
            "i_f_path_disjoint": true,
            "negative_cases": negative_results,
            "failures": failures,
            "status": status
        });
        write_json(&Path::new(DRYRUN_ROOT).join("status_lint.json"), &result)?;
        ensure_pass(&result, "pre-F2.9 status lint")?;
        return Ok(result);
    }
    ...
    let negative_cases = [
        (
            ...
            readme.clone(),
            roadmap.replace(
                &format!("| Gate 1 v2.9 |{}", expected.roadmap_row_tail),
                "| Gate 1 v2.9 | scenario | decision | Complete",
            ),
            baseline.clone(),
        ),
        (
...
fn prefinal_publication_failures(
    registry: &Value,
    readme: &str,
    roadmap: &str,
    baseline: &str,
    ...
    let mut failures = Vec::new();
    if registry["current_experiment"] != "gate1-v2.8"
        || registry["finalization_status"] != "FINALIZED"
    {
        failures.push("published registry is not the immutable F2.8 snapshot".to_owned());
    }
    for (name, source) in [
        ("README", readme),
        ("ROADMAP", roadmap),
        ...
        if !source.contains("gate1-v2.8-status:start")
            || !source.contains("gate1-v2.8-status:end")
            || source.contains("gate1-v2.9-status:start")
            || source.contains("gate1-v2.9-status:end")
        {
            failures.push(format!(
                "{name} was modified before the disjoint F2.9 publication phase"
            ));
        }
    }
    failures
...
impl StatusView {
    fn from_registry(registry: &Value) -> Result<Self, AnyError> {
        ...
        let decision = registry["product_decision"]
            .as_str()
            .or_else(|| registry["decision"].as_str())
        ...
        Ok(Self {
            ...
            roadmap_row_tail: format!(
                " Stable projection and finalization apparatus is prefreeze-complete | Two formal runs and replay preserve real outcomes | {roadmap_status} |"
            ),
        })
    }
}
...
fn status_failures(
    ...
    for (path, source) in [
        ...
        if !source.contains("gate1-v2.9-status:start") || !source.contains("gate1-v2.9-status:end")
        {
            failures.push(format!("{path} has no v2.5 status block"));
        }
    ...
    if !roadmap.contains(&format!("| Gate 1 v2.9 |{}", expected.roadmap_row_tail)) {
    ...
    if expected.experiment != "gate1-v2.9" {
        failures.push("current experiment is not gate1-v2.9".to_owned());
    }
    failures
}
