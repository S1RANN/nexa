...
pub const V2_ROOT: &str = "experiments/gate1-v2.9";
pub const MANIFEST: &str = "experiments/gate1-v2.9/manifest.json";
pub const TARGET_ROOT: &str = "target/gate1-v2.9";

#[derive(Clone, Debug, PartialEq, Eq)]
#[allow(clippy::struct_excessive_bools)]
pub struct DecisionInputs<'a> {
    pub apparatus_status: &'a str,
    pub hypothesis_outcomes: [&'a str; 3],
    pub comparison_outcomes: [&'a str; 2],
    pub stable_core_failure: bool,
    pub structured_pivot_approved: bool,
    pub pilot_committed: bool,
    pub gate2_budget_approved: bool,
}

#[must_use]
pub fn product_decision(inputs: &DecisionInputs<'_>) -> &'static str {
    if matches!(
        inputs.apparatus_status,
        "INVALID" | "STRUCTURAL_FAILURE" | "NOT_RUN"
    ) || inputs.hypothesis_outcomes.contains(&"INVALID")
        || inputs.comparison_outcomes.contains(&"INVALID")
    {
        return "INVALID";
    }
    if inputs.stable_core_failure {
        return if inputs.structured_pivot_approved {
            "PIVOT"
        } else {
            "STOP"
        };
    ...
    if inputs
        .comparison_outcomes
        .iter()
        .any(|outcome| matches!(*outcome, "FAIL" | "INCONCLUSIVE"))
        || inputs
            .hypothesis_outcomes
            .iter()
            ...
            >= 2
    {
        return "UNVERIFIABLE_WITHIN_MVR";
    }
    if !inputs
        .hypothesis_outcomes
        .iter()
        .all(|outcome| *outcome == "PASS")
    {
        return "INVALID";
    }
    if !inputs.pilot_committed {
        "HOLD"
    } else if inputs.gate2_budget_approved {
