...
use nexa_gate1_v2_9_fixtures::{FixtureCase, artifact_bundle};
use serde::Deserialize;
use serde_json::{Value, json};

use crate::scenario;

const DRYRUN_ROOT: &str = "target/gate1-v2.9-dryrun";
const GRAPH_PATH: &str = "reports/history/gate1/supersession_graph.json";
const INDEX_PATH: &str = "reports/history/gate1/index.json";
const CONTRACTS_PATH: &str = "reports/contracts/gate1_v2_9_contracts.json";
...
pub fn history_check() -> Result<Value, AnyError> {
    ...
    let historical_immutability = immutable
        .iter()
        .filter(|(version, _)| version.as_str() != "gate1-v2.9")
        .all(|(_, value)| *value);
    result["metrics"] = json!({
...
pub fn governance_negative_tests() -> Result<Value, AnyError> {
    ...
    cycle
        .edges
        .push(["gate1-v2.9".to_owned(), "gate1-v1".to_owned()]);
    ...
    current_outgoing
        .edges
        .push(["gate1-v2.9".to_owned(), "gate1-v2.2".to_owned()]);
    cases.push(run_case(
    ...
    unknown_edge
        .edges
        .push(["gate1-unknown".to_owned(), "gate1-v2.9".to_owned()]);
...
pub fn h3_execution_independence() -> Result<Value, AnyError> {
    ...
    let traces = specs
        ...
        .map(|spec| serde_json::to_string(&spec.expected_operations))
        .collect::<Result<BTreeSet<_>, _>>()?;
    let formal_source = std::fs::read_to_string("tools/gate1-v2-9/src/main.rs")?;
    let forbidden = [
...
pub fn decision_branches() -> Result<Value, AnyError> {
    ...
    .collect::<Vec<_>>();
    let representative_subgates = json!({
        "H1": ["PASS", "PASS", "PASS"],
        "H2": ["PASS", "PASS", "PASS"],
        "H3": ["PASS", "PASS", "PASS"]
    });
    let per_run = json!({
        "H1": aggregate_prefreeze_components(&[
            ["PASS", "PASS", "PASS"],
            ["FAIL", "FAIL", "FAIL"],
        ]),
        "H2": aggregate_prefreeze_components(&[
            ["PASS", "PASS", "PASS"],
            ["FAIL", "FAIL", "FAIL"],
            ["PASS", "PASS", "PASS"],
            ["PASS", "PASS", "PASS"],
        ...
        "H3": aggregate_prefreeze_components(&[
            ["PASS", "PASS", "PASS"],
            ["FAIL", "FAIL", "FAIL"],
            ["FAIL", "FAIL", "FAIL"],
        ])
    ...
    let stable = ["H1", "H2", "H3"]
        .into_iter()
        .filter(|hypothesis| {
            per_run[*hypothesis].as_array().is_some_and(|values| {
                values.len() == 3 && values.iter().all(|value| value == "FAIL")
            })
        })
        .collect::<Vec<_>>();
    let aggregate_inputs = DecisionInputs {
        apparatus_status: "PASS",
        hypothesis_outcomes: ["FAIL", "FAIL", "FAIL"],
        comparison_outcomes: ["INCONCLUSIVE", "INCONCLUSIVE"],
        stable_core_failure: !stable.is_empty(),
        structured_pivot_approved: false,
        pilot_committed: false,
        gate2_budget_approved: false,
    };
    let actual = product_decision(&aggregate_inputs);
    cases.push(json!({
        "fixture": "representative-subgates-pass-aggregate-hypotheses-fail",
        "representative_subgate_outcomes": representative_subgates,
        "per_run_product_outcomes": per_run,
        "stable_core_failures": stable,
        "expected": "STOP",
        "actual": actual,
        "matched": stable == ["H1", "H2", "H3"] && actual == "STOP",
        "synthetic": true,
        "formal_evidence_usable": false,
...
fn aggregate_prefreeze_components(components: &[[&str; 3]]) -> Vec<String> {
    (0..3)
        .map(|run| {
            let values = components
                .iter()
                .map(|component| component[run])
                .collect::<Vec<_>>();
            if values.contains(&"INVALID") {
                "INVALID"
            } else if values.contains(&"FAIL") {
                "FAIL"
            } else if values.contains(&"INCONCLUSIVE") {
                "INCONCLUSIVE"
            } else {
                "PASS"
            }
...
pub fn synthetic_git_chain() -> Result<Value, AnyError> {
    let root = std::env::temp_dir().join(format!(
        "nexa-gate1-v2.9-synthetic-chain-{}",
        std::process::id()
    ));
    if root.exists() {
    ...
    git_in(
        &root,
        &["config", "user.email", "gate1-v2.9@example.invalid"],
    )?;
    git_in(&root, &["config", "user.name", "Gate 1 v2.9 Dry Run"])?;
    ...
    std::fs::create_dir_all(root.join("reports/raw/gate1_v2_9"))?;
    std::fs::write(
        root.join("reports/raw/gate1_v2_9/synthetic.json"),
    ...
    let d_paths = [
        "reports/contracts/gate1_v2_9_contract_evaluation.json",
        "reports/contracts/gate1_v2_9_results_pending_receipt.json",
        "reports/gate1_v2_9_decision_pending_receipt.md",
        "reports/gate1_v2_9_summary_pending_receipt.md",
        "reports/gate1_v2_9_pilot.json",
        "reports/gate1_v2_9_budget.json",
    ];
    for path in d_paths {
    ...
    let f_paths = [
        "reports/contracts/gate1_v2_9_results.json",
        "reports/contracts/gate1_v2_9_finalization.json",
        "reports/gate1_v2_9_final_decision.md",
        "reports/gate1_v2_9_summary.md",
    ...
    let expected_hashes = final_bytes
        ...
        .collect::<BTreeMap<_, _>>();
    let receipt_path = "reports/contracts/gate1_v2_9_verification_receipt.json";
    ...
    let path_ok = evidence_paths
        .lines()
        .all(|path| path.starts_with("reports/raw/gate1_v2_9/"))
...
pub fn prefreeze_closure() -> Result<Value, AnyError> {
    ...
    let production_decision_status = Command::new("cargo")
        .args([
            "run",
            "--release",
            "-q",
            "-p",
            "nexa-gate1-v2-9-decision",
            "--",
            "decision-state-check",
        ])
        .current_dir(crate::repository_root())
        .status()?;
    if !production_decision_status.success() {
        return Err("production decision-state regression failed".into());
    }
    let production_decisions = read_json(Path::new(DRYRUN_ROOT).join("decision_state_check.json"))?;
    ...
    let qualification =
        read_json("experiments/gate1-v2.9/qualification/environment_qualification.json")?;
    ...
    let result = json!({
        ...
        "checks": {
            ...
            "production_decision_state_check": production_decisions["status"],
    ...
    let frozen_root = Path::new("experiments/gate1-v2.9/prefreeze");
