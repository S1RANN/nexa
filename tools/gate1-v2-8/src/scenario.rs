...
pub const H1_MANIFEST: &str = "experiments/gate1-v2.8/h1_mutations.json";
pub const H1_HANDWRITTEN: &str = "experiments/gate1-v2.8/fixtures/h1_handwritten.rs";
pub const H2_MANIFEST: &str = "experiments/gate1-v2.8/h2_matrix.json";
pub const H2_CLEANUP_MANIFEST: &str = "experiments/gate1-v2.8/h2_cleanup_matrix.json";
pub const H3_MANIFEST: &str = "experiments/gate1-v2.8/h3_scenarios.json";
pub const SCENARIO_SCHEMA: &str = "experiments/gate1-v2.8/scenario_schema.json";
...
pub fn status_lint() -> Result<Value, AnyError> {
    ...
    let negative_cases = [
        (
            ...
            readme.clone(),
            roadmap.replace(
                &format!("| Gate 1 v2.8 |{}", expected.roadmap_row_tail),
                "| Gate 1 v2.8 | scenario | decision | Complete",
            ),
            baseline.clone(),
        ),
        (
...
fn status_failures(
    ...
    for (path, source) in [
        ...
        if !source.contains("gate1-v2.8-status:start") || !source.contains("gate1-v2.8-status:end")
        {
    ...
    if !roadmap.contains(&format!("| Gate 1 v2.8 |{}", expected.roadmap_row_tail)) {
    ...
    if expected.experiment != "gate1-v2.8" {
        failures.push("current experiment is not gate1-v2.8".to_owned());
    }
    failures
}
