...
const ACCEPTANCE: &str = "baseline/testing/GATE1_ACCEPTANCE_V2_8.md";
const AUTHORIZATION: &str = "baseline/testing/GATE1_V2_8_AUTHORIZATION.md";
const AUTHORIZATION_RECORD: &str = "experiments/gate1-v2.8/authorization.json";
const THRESHOLDS: &str = "experiments/gate1-v2.8/threshold_equivalence.json";
const ENVIRONMENT: &str = "experiments/gate1-v2.8/environment.json";
const QUALIFICATION: &str = "experiments/gate1-v2.8/qualification/environment_qualification.json";
...
fn main() -> Result<(), AnyError> {
    ...
    match arguments.as_slice() {
        ...
        [command] if command == "qualify-environment" => qualification::qualify_environment(
            Path::new("target/gate1-v2.8-qualification/qualified-host"),
        ),
        ...
        [command] if command == "archive-v2-5" => {
            let result = history_v2_5::archive()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "archive-v2-6" => {
            let result = history_v2_6::archive()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "h2-projection-check" => {
            let result = projection::projection_check()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "h2-noise-invariance-check" => {
            let result = projection::noise_invariance_check()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "h2-semantic-sensitivity-check" => {
            let result = projection::semantic_sensitivity_check()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "comparison-policy-check" => {
            let result = projection::comparison_policy_check()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "comparison-truth-table" => {
            let result = projection::comparison_truth_table()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "inconclusive-contract-check" => {
            let result = projection::inconclusive_contract_check()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "stable-failure-check"
            || command == "decision-priority-check" =>
        {
            let result = prefreeze::decision_branches()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "decision-state-check" => {
            let result = prefreeze::decision_branches()?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            Ok(())
        }
        [command] if command == "topology-finalization-check"
            || command == "finalization-topology-check" =>
        {
            let topology = prefreeze::synthetic_git_chain()?;
            let finalization = prefreeze::terminal_short_circuit()?;
            let result = json!({
                "schema_version": 1,
                "experiment_version": "gate1-v2.8",
                "topology": topology,
                "finalization": finalization,
                "status": if topology["status"] == "PASS" && finalization["status"] == "PASS" {
                    "PASS"
                } else {
                    "FAIL"
                }
            ...
            println!("{}", serde_json::to_string_pretty(&result)?);
            ensure_pass(&result, "topology finalization")
        }
        [command] if command == "artifact-hygiene-check" => {
            let result =
                scenario::verify_artifact_hygiene(Path::new("target/gate1-v2.8-dryrun"))?;
            println!("{}", serde_json::to_string_pretty(&result)?);
            ensure_pass(&result, "artifact hygiene")
        }
        ...
        _ => Err("usage: nexa-gate1-v2-8 history-check|status-lint|acceptance-equivalence|comparison-truth-table|inconclusive-contract-check|stable-failure-check|decision-priority-check|finalization-topology-check|artifact-hygiene-check|evidence-static-check|qualify-environment|prefreeze-closure|freeze|validate|supervisor <formal-run-1|formal-run-2|replay>|compare <left> <right>".into()),
    }
}
...
fn acceptance_equivalence() -> Result<(), AnyError> {
    ...
    if weakened
        || !removed.is_empty()
        || !changed.is_empty()
        || fingerprint["experiment_version"] != "gate1-v2.8"
        ...
        || fingerprint["authorized_apparatus_changes"]
            != json!([
                "legal INCONCLUSIVE to contract PASS mapping",
                "structured comparison outcome schema",
                "stable core failure decision priority",
                "five-commit finalization protocol",
                "receipt precomputed final-file hashes",
                "historical v2.5 failure sealing",
                "historical v2.6 structural invalidation",
                "H2 Raw pointer repair",
                "per-run derived-to-recorded component validation",
                "direct H2 count and absolute-zero Contract assertions",
                "absolute global allocator delta semantics",
                "historical v2.7 invalid environment execution sealing",
                "benchmark warmup metric schema repair",
                "unrestricted qualified formal execution enforcement"
            ])
    {
        return Err("Gate 1 v2.8 outcome thresholds are not equivalent to v2".into());
    }
    println!(
        "{}",
...
fn evidence_static_check() -> Result<(), AnyError> {
    let roots = [
        Path::new("tools/gate1-v2-8/src"),
        Path::new("tools/gate1-v2-8-gates/src"),
        Path::new("tools/gate1-v2-8-decision/src"),
...
fn freeze() -> Result<(), AnyError> {
    ...
    let prefreeze_closure = read_json("experiments/gate1-v2.8/prefreeze/prefreeze_closure.json")?;
    if prefreeze_closure["status"] != "PASS"
        || prefreeze_closure["synthetic_artifacts_formal_evidence_usable"] != false
    {
        return Err("Gate 1 v2.8 prefreeze closure is not valid".into());
    }
    ...
    {
        return Err("Gate 1 v2.8 environment is not qualified".into());
    }
    let authorization = read_json(AUTHORIZATION_RECORD)?;
    if authorization["status"] != "AUTHORIZED"
        || authorization["prefreeze_closure"]["hash"]
            != hash_file("experiments/gate1-v2.8/prefreeze/prefreeze_closure.json")?
        ...
        || authorization["provenance_protocol"]["hash"]
            != hash_file("experiments/gate1-v2.8/prefreeze/outcome_transport.json")?
        || authorization["execution_budget"]
            != json!({"formal_run_1": 1, "formal_run_2": 1, "replay": 1, "retry": 0})
    {
        return Err("Gate 1 v2.8 authorization bindings are invalid".into());
    }
    let contract_manifest = read_json("reports/contracts/gate1_v2_8_contracts.json")?;
    if contract_manifest["contracts"]
        .as_array()
        .map_or(0, Vec::len)
        != 50
    {
        return Err("Gate 1 v2.8 Contract Manifest does not cover 50 work packages".into());
    }
    ...
    let runner_hash = stable_value_hash(&json!({
        "runner_lib": hashes["tools/gate1-v2-8/src/lib.rs"],
        "runner_main": hashes["tools/gate1-v2-8/src/main.rs"],
        "gates_main": hashes["tools/gate1-v2-8-gates/src/main.rs"],
        "gates_lib": hashes["tools/gate1-v2-8-gates/src/lib.rs"],
        "decision": hashes["tools/gate1-v2-8-decision/src/main.rs"],
        ...
        ,"qualification": hashes["tools/gate1-v2-8/src/qualification.rs"]
        ,"prefreeze": hashes["tools/gate1-v2-8/src/prefreeze.rs"]
        ,"scenario": hashes["tools/gate1-v2-8/src/scenario.rs"]
        ,"projection": hashes["tools/gate1-v2-8/src/projection.rs"]
        ,"history_v2_5": hashes["tools/gate1-v2-8/src/history_v2_5.rs"]
        ,"history_v2_6": hashes["tools/gate1-v2-8/src/history_v2_6.rs"]
        ,"fixtures_lib": hashes["tools/gate1-v2-8-fixtures/src/lib.rs"]
        ,"fixtures_main": hashes["tools/gate1-v2-8-fixtures/src/main.rs"]
    }));
    let manifest = json!({
        "schema_version": 3,
        "experiment_version": "gate1-v2.8",
        ...
        "prefreeze_closure_hash": hashes["experiments/gate1-v2.8/prefreeze/prefreeze_closure.json"],
        "supersession_graph_hash": hashes["reports/history/gate1/supersession_graph.json"],
        "sample_hash": sample_hash,
        "sample_hashes": sample_hashes,
        "runner_hash": runner_hash,
        "worker_hash": hashes["tools/gate1-v2-8/src/main.rs"],
        "decision_tool_hash": hashes["tools/gate1-v2-8-decision/src/main.rs"],
        "receipt_generator_hash": hashes["tools/gate1-v2-8-decision/src/main.rs"],
        "h2_benchmark_config_hash": hashes["experiments/gate1/benchmark.json"],
        "contract_manifest_hash": hashes["reports/contracts/gate1_v2_8_contracts.json"],
        "bound_hashes": hashes
    });
    write_json(Path::new(MANIFEST), &manifest)?;
    println!(
        "froze Gate 1 v2.8 manifest with {} bound inputs",
        paths.len()
    );
    Ok(())
}
...
fn bound_input_paths() -> Vec<String> {
    let mut paths = [
        ...
        ENVIRONMENT,
        QUALIFICATION,
        "experiments/gate1-v2.8/qualification/environment_qualification.md",
        "experiments/gate1-v2.8/qualification/environment_qualification_hashes.json",
        "experiments/gate1-v2.8/qualification/root-cause.json",
        "experiments/gate1-v2.8/qualification/formal-handshake/process_attestation.json",
        "experiments/gate1-v2.8/qualification/formal-handshake/parent_verification.json",
        "experiments/gate1-v2.8/qualification/formal-handshake/probe.json",
        "experiments/gate1-v2.8/prefreeze/history_check.json",
        "experiments/gate1-v2.8/prefreeze/governance_negative_tests.json",
        "experiments/gate1-v2.8/prefreeze/status_lint.json",
        "experiments/gate1-v2.8/prefreeze/scenario_independence.json",
        "experiments/gate1-v2.8/prefreeze/outcome_transport.json",
        "experiments/gate1-v2.8/prefreeze/h1_transformer_equivalence.json",
        "experiments/gate1-v2.8/prefreeze/h2_dimension_effectiveness.json",
        "experiments/gate1-v2.8/prefreeze/h2_cleanup_independence.json",
        "experiments/gate1-v2.8/prefreeze/h3_execution_independence.json",
        "experiments/gate1-v2.8/prefreeze/raw_regeneration_exercise.json",
        "experiments/gate1-v2.8/prefreeze/contract_satisfiability.json",
        "experiments/gate1-v2.8/prefreeze/decision_branches.json",
        "experiments/gate1-v2.8/prefreeze/terminal_short_circuit.json",
        "experiments/gate1-v2.8/prefreeze/synthetic_git_chain.json",
        "experiments/gate1-v2.8/prefreeze/structural_failure_regression.json",
        "experiments/gate1-v2.8/prefreeze/h2_projection.json",
        "experiments/gate1-v2.8/prefreeze/h2_noise_invariance.json",
        "experiments/gate1-v2.8/prefreeze/h2_semantic_sensitivity.json",
        "experiments/gate1-v2.8/prefreeze/comparison_policy.json",
        "experiments/gate1-v2.8/prefreeze/comparison_truth_table.json",
        "experiments/gate1-v2.8/prefreeze/inconclusive_contract.json",
        "experiments/gate1-v2.8/prefreeze/prefreeze_closure.json",
        "experiments/gate1-v2.8/h2_semantic_projection.json",
        "experiments/gate1-v2.8/h2_performance_policy.json",
        "experiments/gate1-v2.8/decision_state_machine.json",
        "experiments/gate1-v2.8/fixtures/v2_5_h2/formal-run-1.json",
        "experiments/gate1-v2.8/fixtures/v2_5_h2/formal-run-2.json",
        "experiments/gate1-v2.8/fixtures/v2_5_h2/replay.json",
        "experiments/gate1-v2.8/fixtures/v2_5_comparisons/formal.json",
        "experiments/gate1-v2.8/fixtures/v2_5_comparisons/replay.json",
        ...
        "reports/history/gate1/v2_5/terminal.md",
        ...
        "reports/history/gate1/v2_6/terminal.md",
        ...
        "reports/history/gate1/v2_7/terminal.md",
        ...
        "tools/gate1-v2-8/src/lib.rs",
        "tools/gate1-v2-8/src/main.rs",
        "tools/gate1-v2-8/src/history_v2_5.rs",
        "tools/gate1-v2-8/src/history_v2_6.rs",
        "tools/gate1-v2-8/src/projection.rs",
        "tools/gate1-v2-8/src/qualification.rs",
        "tools/gate1-v2-8/src/prefreeze.rs",
        "tools/gate1-v2-8/src/scenario.rs",
        "tools/gate1-v2-8-fixtures/src/lib.rs",
        "tools/gate1-v2-8-fixtures/src/main.rs",
        "tools/gate1-v2-8-gates/src/main.rs",
        "tools/gate1-v2-8-gates/src/lib.rs",
        "tools/gate1-v2-8-decision/src/main.rs",
        ...
        "reports/contracts/gate1_v2_8_contracts.json",
    ]
    .into_iter()
    .map(str::to_owned)
    .collect::<Vec<_>>();
...
fn supervisor(label: &str) -> Result<(), AnyError> {
    ...
    if result["apparatus_status"] != "PASS" {
        return Err(format!("Gate 1 v2.8 {label} was invalid: {}", result["failures"]).into());
    }
    println!(
        "Gate 1 v2.8 {label}: apparatus PASS, outcomes {}",
...
fn h1_worker(label: &str, supplied_nonce: &str) -> Result<(), AnyError> {
    ...
    let process = recorder.finish()?;
    events.sync()?;
    let equivalence_pass = artifacts.len() == 20
        && artifacts
            .iter()
            .all(|artifact| artifact["semantic_equivalence"] == true);
    let metrics_pass = idl.functions.len() == 20
        && artifacts.len() == 20
        && line_reduction >= 50
        && edit_reduction >= 50
        && early_rejections == 20
    ...
    let result = json!({
        ...
        "failures": failures,
        "component_outcomes": {
            "h1_equivalence": if equivalence_pass {"PASS"} else {"FAIL"},
            "h1_metrics": if metrics_pass {"PASS"} else {"FAIL"}
        },
        "metrics": {
...
fn run_h1_mutation(
    ...
    let hand_build = cargo_check_binding(
        ...
        &hand_file,
        &repository_root()
            .join("target/gate1-v2.8-build/h1")
            .join(format!("{id}-hand")),
        false,
    )?;
        ...
        Ok(changed_idl) => {
            ...
            if build.status.success() {
                ...
                let module = nexa_compiler::compile_with_metadata(
                    ...
                    changed_hash,
                    StableId::from_name("gate1-v2.8-h1-schema"),
                )?;
...
fn h2_worker(label: &str, supplied_nonce: &str) -> Result<(), AnyError> {
    ...
    let configuration_pass = scenarios.len() == 32
    ...
    let cleanup_pass = cleanup.len() == 12
        && cleanup.iter().all(|case| case["status"] == "observed")
        && cleanup
            .iter()
            ...
            .len()
            == 12;
    let invariants_pass = scenarios
        .iter()
        .all(|scenario| scenario["violations"].as_array().is_some_and(Vec::is_empty));
    let allocation_pass = allocator_failures.is_empty()
        && observer_value["runs"].as_array().is_some_and(|runs| {
            runs.len() == 3
                && runs.iter().all(|run| {
                    [
                        "promotion",
                    ...
                    .iter()
                    .all(|field| run[*field].as_u64() == Some(0))
                })
        })
        && allocation_counts.values().all(|count| *count == 0);
    let performance_pass = timing_failures.is_empty()
        && benchmark_processes.len() == 3
        && benchmark_processes
            .iter()
            .all(|process| process["samples"].as_u64() == Some(1000));
    reserve_result_path(&root, label)?;
    let process = recorder.finish()?;
    events.sync()?;
    ...
    let mut result = json!({
        ...
        "component_outcomes": {
            "h2_configuration": if configuration_pass {"PASS"} else {"FAIL"},
            "h2_cleanup": if cleanup_pass {"PASS"} else {"FAIL"},
            "h2_invariants": if invariants_pass {"PASS"} else {"FAIL"},
            "h2_allocations": if allocation_pass {"PASS"} else {"FAIL"},
            "h2_performance": if performance_pass {"PASS"} else {"FAIL"}
        },
        "metrics": {
    ...
    result["semantic_projection"] = projection::h2_semantic_projection(&result)?;
    result["semantic_signature"] = json!(projection::h2_semantic_signature(&result)?);
    result["performance_projection"] = projection::h2_performance_projection(&result)?;
    result["allocation_projection"] = projection::h2_allocation_projection(&result)?;
    write_json(&root.join("result.json"), &result)?;
    Ok(())
}
...
fn h3_worker(label: &str, supplied_nonce: &str) -> Result<(), AnyError> {
    ...
    let group_outcome = |group: &str, expected: usize| {
        if matrices[group].as_array().is_some_and(|scenarios| {
            scenarios.len() == expected
                && scenarios
                    .iter()
                    .all(|scenario| scenario["actual_outcome"] == "PASS")
        }) {
            "PASS"
        } else {
            "FAIL"
    ...
    let result = json!({
        ...
        "failures": failures,
        "component_outcomes": {
            "h3_migration": group_outcome("migration", 11),
            "h3_completion": group_outcome("completion", 10),
            "h3_transaction": group_outcome("transaction", 9)
        },
        "metrics": {
...
fn compare(left: &str, right: &str) -> Result<(), AnyError> {
    ...
    let h1 = compare_h1(&left_value["h1"], &right_value["h1"]);
    let h2 = projection::compare_h2(&left_value["h2"], &right_value["h2"])
        .unwrap_or_else(|error| json!({"status": "INVALID", "error": error.to_string()}));
    let h3 = compare_h3(&left_value["h3"], &right_value["h3"]);
    let mut failures = Vec::new();
    for (hypothesis, comparison) in [("h1", &h1), ("h2", &h2), ("h3", &h3)] {
        if comparison["status"] != "PASS" {
            failures.push(format!(
                "{hypothesis} comparison is {}",
                comparison["status"].as_str().unwrap_or("INVALID")
            ));
        }
    }
    let statuses = [&h1, &h2, &h3].map(|value| value["status"].as_str().unwrap_or("INVALID"));
    let status = if statuses.contains(&"INVALID") {
        "INVALID"
    ...
    println!("{left} vs {right}: {status}");
    Ok(())
}

fn compare_h1(left: &Value, right: &Value) -> Value {
    let left_artifacts = left["artifacts"].as_array().map_or(&[][..], Vec::as_slice);
    let right_artifacts = right["artifacts"].as_array().map_or(&[][..], Vec::as_slice);
    let schema_valid = left_artifacts.len() == 20
        && right_artifacts.len() == 20
        && left["semantic_signature"].is_string()
        && right["semantic_signature"].is_string()
        && left["outcome"].is_string()
        && right["outcome"].is_string();
    let semantic_equal = schema_valid
        && left["semantic_signature"] == right["semantic_signature"]
        && left_artifacts
            .iter()
            .map(h1_semantic_artifact)
            .eq(right_artifacts.iter().map(h1_semantic_artifact));
    let outcome_equal = left["outcome"] == right["outcome"];
    json!({
        "status": if !schema_valid {"INVALID"} else if semantic_equal && outcome_equal {"PASS"} else {"FAIL"},
        "schema_valid": schema_valid,
        "semantic_equal": semantic_equal,
        "outcome_equal": outcome_equal,
        "left_signature": left["semantic_signature"],
        "right_signature": right["semantic_signature"],
        "left_outcome": left["outcome"],
        "right_outcome": right["outcome"],
        "mutation_count": left_artifacts.len()
    })
}

fn compare_h3(left: &Value, right: &Value) -> Value {
    let left_count = h3_scenario_count(left);
    let right_count = h3_scenario_count(right);
    let schema_valid = left_count == 30
        && right_count == 30
        && left["semantic_signature"].is_string()
        && right["semantic_signature"].is_string()
        && left["outcome"].is_string()
        && right["outcome"].is_string();
    let semantic_equal = schema_valid && left["semantic_signature"] == right["semantic_signature"];
    let outcome_equal = left["outcome"] == right["outcome"];
    json!({
        "status": if !schema_valid {"INVALID"} else if semantic_equal && outcome_equal {"PASS"} else {"FAIL"},
        "schema_valid": schema_valid,
        ...
        "left_outcome": left["outcome"],
        "right_outcome": right["outcome"],
        "scenario_count": left_count
    })
}

fn h3_scenario_count(value: &Value) -> usize {
    ["migration", "completion", "transaction"]
        .into_iter()
        .map(|group| value["matrices"][group].as_array().map_or(0, Vec::len))
        .sum()
}
    ...
    for spec in scenario::h2_cleanup_specs()? {
        ...
        let (failure_point, sequence, panic_contained) = match spec.executor.as_str() {
            ...
            "execute_host_panic" => (
                ...
                Some(std::panic::catch_unwind(|| panic!("gate1-v2.8 host panic fixture")).is_err()),
            ),
            "execute_cancel" => (
                None,
                vec![
...
fn synthetic_failed_output(message: String) -> Output {
    ...
    {
        let _ = message;
        unreachable!("Nexa Gate 1 v2.8 formal apparatus currently targets Unix hosts")
    }
}
...
fn formal_handshake_probe() -> Result<(), AnyError> {
    let root = repository_root().join("target/gate1-v2.8-qualification/formal-handshake");
    if root.exists() {
...
fn require_run_label(label: &str) -> Result<(), AnyError> {
    if matches!(label, "formal-run-1" | "formal-run-2" | "replay") {
        ...
        Err(format!("unauthorized Gate 1 v2.8 run label `{label}`").into())
    }
}
