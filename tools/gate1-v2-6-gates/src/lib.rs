...
use nexa_gate1_v2_6::{AnyError, hash_file, read_json, stable_value_hash, write_json};
use serde_json::{Value, json};

pub const RAW_ROOT: &str = "reports/raw/gate1_v2_6";
...
const STATIC_INPUTS: [&str; 43] = [
    "experiments/gate1-v2.6/qualification/environment_qualification.json",
    ...
    "experiments/gate1-v2.6/threshold_equivalence.json",
    ...
    "experiments/gate1-v2.6/pilot.json",
    "experiments/gate1-v2.6/budget.json",
...
pub fn package_all() -> Result<(), AnyError> {
    ...
    if raw.exists() {
        return Err(
            "reports/raw/gate1_v2_6 already exists; Evidence packaging is immutable".into(),
        );
    }
    for run in RUNS {
        copy_evidence_tree(
            &Path::new("target/gate1-v2.6").join(run),
            &raw.join("runs").join(run),
        )?;
    }
    copy_evidence_tree(
        Path::new("target/gate1-v2.6/comparisons"),
    ...
    for source in [
        "experiments/gate1-v2.6/h1_mutations.json",
        "experiments/gate1-v2.6/h2_matrix.json",
        "experiments/gate1-v2.6/h2_cleanup_matrix.json",
        "experiments/gate1-v2.6/h3_scenarios.json",
        "experiments/gate1-v2.6/scenario_schema.json",
    ] {
...
pub fn rebuild_and_compare() -> Result<(), AnyError> {
    ...
    let regenerated = Path::new("target/gate1-v2.6-gate-rebuild");
...
pub fn artifact_hygiene() -> Result<(), AnyError> {
    ...
    println!("Gate 1 v2.6 Evidence artifact hygiene: PASS");
    Ok(())
}
...
fn build_gates(
    ...
    let history_check = static_json(raw, "experiments/gate1-v2.6/prefreeze/history_check.json")?;
    ...
    let history_failures = failures([
        ...
        (
            current["current_experiment"] == "gate1-v2.6",
            "v2.6 is not the unique current experiment",
        ),
    ]);
    let history = gate(
        "history",
    ...
    let authorization = static_json(raw, "experiments/gate1-v2.6/authorization.json")?;
    let thresholds = static_json(raw, "experiments/gate1-v2.6/threshold_equivalence.json")?;
    ...
    let pilot_value = static_json(raw, "experiments/gate1-v2.6/pilot.json")?;
    let pilot = gate(
        "pilot",
        "EXTERNAL",
        &[],
    ...
    let budget_value = static_json(raw, "experiments/gate1-v2.6/budget.json")?;
    let budget = gate(
        "budget",
        "EXTERNAL",
        &[],
    ...
    let h2_projection = static_json(raw, "experiments/gate1-v2.6/prefreeze/h2_projection.json")?;
    ...
    let status_lint = static_json(raw, "experiments/gate1-v2.6/prefreeze/status_lint.json")?;
...
fn comparison_gate(
    ...
    json!({
        ...
        "implementation_sha": implementation_sha,
        "implementation_tree": implementation_tree,
        "generator": "nexa-gate1-v2-6-gates/raw-v2"
    })
}
...
fn gate(
    ...
    json!({
        ...
        "implementation_sha": implementation_sha,
        "implementation_tree": implementation_tree,
        "generator": "nexa-gate1-v2-6-gates/raw-v1"
    })
}
