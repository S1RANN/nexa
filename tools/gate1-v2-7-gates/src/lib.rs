...
use nexa_gate1_v2_7::{AnyError, hash_file, read_json, stable_value_hash, write_json};
use serde_json::{Value, json};

pub const RAW_ROOT: &str = "reports/raw/gate1_v2_7";
...
const STATIC_INPUTS: [&str; 47] = [
    "experiments/gate1-v2.7/qualification/environment_qualification.json",
    ...
    "experiments/gate1-v2.7/threshold_equivalence.json",
    ...
    "experiments/gate1-v2.7/pilot.json",
    "experiments/gate1-v2.7/budget.json",
...
pub fn package_all() -> Result<(), AnyError> {
    ...
    if raw.exists() {
        return Err(
            "reports/raw/gate1_v2_7 already exists; Evidence packaging is immutable".into(),
        );
    }
    for run in RUNS {
        copy_evidence_tree(
            &Path::new("target/gate1-v2.7").join(run),
            &raw.join("runs").join(run),
        )?;
    }
    copy_evidence_tree(
        Path::new("target/gate1-v2.7/comparisons"),
    ...
    for source in [
        "experiments/gate1-v2.7/h1_mutations.json",
        "experiments/gate1-v2.7/h2_matrix.json",
        "experiments/gate1-v2.7/h2_cleanup_matrix.json",
        "experiments/gate1-v2.7/h3_scenarios.json",
        "experiments/gate1-v2.7/scenario_schema.json",
    ] {
...
pub fn rebuild_and_compare() -> Result<(), AnyError> {
    ...
    let regenerated = Path::new("target/gate1-v2.7-gate-rebuild");
...
pub fn artifact_hygiene() -> Result<(), AnyError> {
    ...
    println!("Gate 1 v2.7 Evidence artifact hygiene: PASS");
    Ok(())
}
...
fn build_gates(
    ...
    let history_check = static_json(raw, "experiments/gate1-v2.7/prefreeze/history_check.json")?;
    ...
    let history_failures = failures([
        ...
        (
            current["current_experiment"] == "gate1-v2.7",
            "v2.7 is not the unique current experiment",
        ),
    ]);
    let history = gate(
        "history",
    ...
    let authorization = static_json(raw, "experiments/gate1-v2.7/authorization.json")?;
    let thresholds = static_json(raw, "experiments/gate1-v2.7/threshold_equivalence.json")?;
    ...
    let h2_performance = outcome_gate_with_contract(
        ...
        json!({
            ...
            "warmup_samples": h2.iter().map(|value| value["performance_processes"].as_array().map_or(Vec::new(), |items| items.iter().map(|item| item["warmup"].clone()).collect())).collect::<Vec<Vec<Value>>>(),
            "timed_samples": h2.iter().map(|value| value["performance_processes"].as_array().map_or(Vec::new(), |items| items.iter().map(|item| item["samples"].clone()).collect())).collect::<Vec<Vec<Value>>>()
        }),
        implementation_sha,
        implementation_tree,
    );
    ...
    let pilot_value = static_json(raw, "experiments/gate1-v2.7/pilot.json")?;
    let pilot = gate(
        "pilot",
        "EXTERNAL",
        &[],
    ...
    let budget_value = static_json(raw, "experiments/gate1-v2.7/budget.json")?;
    let budget = gate(
        "budget",
        "EXTERNAL",
        &[],
    ...
    let h2_projection = static_json(raw, "experiments/gate1-v2.7/prefreeze/h2_projection.json")?;
    ...
    let status_lint = static_json(raw, "experiments/gate1-v2.7/prefreeze/status_lint.json")?;
...
fn comparison_gate(
    ...
    json!({
        ...
        "implementation_sha": implementation_sha,
        "implementation_tree": implementation_tree,
        "generator": "nexa-gate1-v2-7-gates/raw-v2"
    })
}
        ...
        .is_some_and(|processes| {
            processes.len() == 3
                && processes.iter().all(|process| {
                    process["warmup"].as_u64() == Some(100)
                        && process["samples"].as_u64() == Some(1_000)
                })
        })
}
...
fn gate(
    ...
    json!({
        ...
        "implementation_sha": implementation_sha,
        "implementation_tree": implementation_tree,
        "generator": "nexa-gate1-v2-7-gates/raw-v1"
    })
}
