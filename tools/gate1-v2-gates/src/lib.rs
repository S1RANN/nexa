...
pub const RAW_ROOT: &str = "reports/raw/gate1_v2_4";
...
const STATIC_INPUTS: [&str; 18] = [
    "experiments/gate1-v2.4/qualification/environment_qualification.json",
    ...
    "experiments/gate1-v2.4/threshold_equivalence.json",
    "experiments/gate1-v2.4/pilot.json",
    "experiments/gate1-v2.4/budget.json",
...
pub fn package_all() -> Result<(), AnyError> {
    ...
    if raw.exists() {
        return Err(
            "reports/raw/gate1_v2_4 already exists; Evidence packaging is immutable".into(),
        );
    }
    for run in RUNS {
        copy_evidence_tree(
            &Path::new("target/gate1-v2.4").join(run),
            &raw.join("runs").join(run),
        )?;
    }
    copy_evidence_tree(
        Path::new("target/gate1-v2.4/comparisons"),
    ...
    for source in [
        "experiments/gate1-v2.4/h1_mutations.json",
        "experiments/gate1-v2.4/h2_matrix.json",
        "experiments/gate1-v2.4/h2_cleanup_matrix.json",
        "experiments/gate1-v2.4/h3_scenarios.json",
        "experiments/gate1-v2.4/scenario_schema.json",
    ] {
...
fn build_gates(
    ...
    let history_check = static_json(raw, "experiments/gate1-v2.4/prefreeze/history_check.json")?;
    ...
    let history_failures = failures([
        ...
        (
            current["current_experiment"] == "gate1-v2.4",
            "v2.4 is not the unique current experiment",
        ),
    ]);
    let history = gate(
        "history",
    ...
    let authorization = static_json(raw, "experiments/gate1-v2.4/authorization.json")?;
    let thresholds = static_json(raw, "experiments/gate1-v2.4/threshold_equivalence.json")?;
    ...
    let process_failures = supervisors
        .iter()
        .enumerate()
        .filter(|(_, supervisor)| supervisor["apparatus_status"] != "PASS")
        .map(|(index, _)| format!("{} supervisor apparatus is not PASS", RUNS[index]))
        .collect::<Vec<_>>();
    let process = gate(
        "process",
        "APPARATUS",
        ...
        json!({
            ...
            "independent_supervisor_nonces": supervisors.iter().filter_map(|value| value["supervisor_nonce"].as_str()).collect::<BTreeSet<_>>().len(),
            "worker_topologies": supervisors.iter().map(|value| value["process_topology"].clone()).collect::<Vec<_>>()
        }),
        implementation_sha,
        implementation_tree,
    );

    let validity_outcome = if process_failures.is_empty() {
        "PASS"
    } else {
        "INVALID"
    };
    let validity = gate(
        ...
        validity_outcome,
        json!({
            "apparatus_valid": process_failures.is_empty(),
            "formal_run_count": supervisors.len(),
            "preflight_postflight_present": supervisors.iter().all(|value| value.get("validity").is_some())
        }),
        implementation_sha,
        implementation_tree,
    );
    ...
    let pilot_value = static_json(raw, "experiments/gate1-v2.4/pilot.json")?;
    let pilot = gate(
        "pilot",
        "EXTERNAL",
        &[],
    ...
    let budget_value = static_json(raw, "experiments/gate1-v2.4/budget.json")?;
    let budget = gate(
        "budget",
        "EXTERNAL",
        &[],
    ...
    let workspace_failures = failures([
        ...
        (
            ...
            "44-contract satisfiability check failed",
        ),
        (
...
fn gate(
    ...
    json!({
        ...
        "implementation_sha": implementation_sha,
        "implementation_tree": implementation_tree,
        "generator": "nexa-gate1-v2-4-gates/raw-v1"
    })
}
