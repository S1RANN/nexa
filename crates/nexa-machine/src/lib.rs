...
impl MachineSpec {
    pub fn parse(source: &str) -> Result<Self, Vec<SpecError>> {
        ...
        for (zero_based_line, raw_line) in source.lines().enumerate() {
            ...
            match words.first().copied() {
                ...
                Some("invariant") => parse_single_name(&words, line_number, "invariant")
                    .map_or_else(|error| errors.push(error), |name| invariants.push(name)),
    ...
    pub fn transition_id(&self, transition: &TransitionSpec) -> StableId {
        StableId::from_name(&transition.name)
    }
    ...
    pub fn generate_rust(&self) -> String {
        ...
        writeln!(output, "        GuardRejected(&'static str),").unwrap();
        writeln!(
            output,
        ...
        writeln!(output, "        guard: impl Fn(&str) -> bool,").unwrap();
        ...
        for (index, transition) in self.transitions.iter().enumerate() {
            ...
            for guard in &transition.guards {
                writeln!(
                    output,
                    "                if !guard(\"{guard}\") {{ return Err(TransitionError::GuardRejected(\"{guard}\")); }}"
                )
                .unwrap();
            }
...
invariant terminal_has_no_resources
