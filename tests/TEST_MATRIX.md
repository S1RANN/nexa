...
|---|---|---|---|---|---|
| `nexa-runtime/tests/task_lifecycle.rs` | Task Lifecycle, Resource Lifecycle, Gameplay Integration | 12 required lifecycle outcomes, handle safety, terminal ledger | Public Task or resource contract regressed | Yes | Failure probes in capacity/cleanup cases |
| `nexa-runtime/tests/restart_reload.rs` | Restart Reload, Resource Lifecycle | 10 restart, migration, late-completion, and activation outcomes | Restart Reload semantics regressed | Yes | Activation probe only |
| `nexa-runtime/tests/runtime_baseline.rs` and `runtime-baseline/fast_complete.rs` | Task Lifecycle | Fast completion and terminal record | Interpreter-driven completion changed | `poll_task_raw` baseline oracle | No |
...
| `nexa-model/tests/realm_v4_differential.rs` | Model Differential | Resource and reload transition oracle | Runtime/model mismatch | No | Model |
| `nexa-model/tests/realm_v5_differential.rs` | Model Differential | Full Realm event state oracle | Runtime/model mismatch | No | Model |
| `nexa-model/tests/realm_v5_failure_differential.rs` | Model Differential | Failure classification matches model | Injected failure escaped its class | No | Model and fault injection |
