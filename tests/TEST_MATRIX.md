...
| `nexa-runtime/tests/public_api_compile.rs` | Task Lifecycle, Public API | External crates cannot create Requests, force Waiting, or call removed raw Task APIs | Lifecycle bypass reopened | Compile-fail | No |
| `examples/combat-runtime` unit test | Host Binding, Resource Lifecycle, Gameplay Integration | Generated Binding releases Request/Token/Snapshot exactly once across completion, cancel, and Reload; typed Snapshot decodes | Real Host resource contract regressed | Yes | Activation probe |
...
| `fuzz/state-schema`, `fuzz/realm-events` | Fuzz, Restart Reload | State schema, migration arena, and real-`RealmRuntime` event/resource sequences | Panic or invariant violation | Real adapter | Fuzz/model |
